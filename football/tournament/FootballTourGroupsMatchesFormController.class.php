...
class FootballTourGroupsMatchesFormController extends DefaultModuleController
{
    private $compet;
    private $params;
    private $match;
	...
	private function build_form()
	{
        ...
        for ($i = 1; $i <= $groups_number; $i++)
        {
            ...
            for ($j = 1; $j <= $matches_number; $j++)
            {
                ...
                $match_date = FootballMatchService::get_match_in_group($this->compet_id(), 'G', $i, $j) ? $this->get_match('G', $i, $j)->get_match_date() : new Date();
                $match_playground = FootballMatchService::get_match_in_group($this->compet_id(), 'G', $i, $j) ? $this->get_match('G', $i, $j)->get_match_playground() : '';
                $match_home_id = FootballMatchService::get_match_in_group($this->compet_id(), 'G', $i, $j) ? $this->get_match('G', $i, $j)->get_match_home_id() : 0;
                $match_home_score = FootballMatchService::get_match_in_group($this->compet_id(), 'G', $i, $j) ? $this->get_match('G', $i, $j)->get_match_home_score() : '';
                $match_away_score = FootballMatchService::get_match_in_group($this->compet_id(), 'G', $i, $j) ? $this->get_match('G', $i, $j)->get_match_away_score() : '';
                $match_away_id = FootballMatchService::get_match_in_group($this->compet_id(), 'G', $i, $j) ? $this->get_match('G', $i, $j)->get_match_away_id() : 0;
	...
	{
        $groups_number = (int)($this->teams_number / $this->teams_per_group);
        if ($this->return_matches)
            $matches_number = $this->teams_per_group * ($this->teams_per_group - 1);
        else
            $matches_number = $this->teams_per_group * ($this->teams_per_group - 1) / 2;
	...
	private function get_match($type, $group, $order)
	{
        ...
        $id = FootballMatchService::get_match_in_group($compet_id, $type, $group, $order)->get_id_match();
