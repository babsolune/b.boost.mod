...
class FootballTourBracketFormController extends DefaultModuleController
{
    private $compet;
    private $params;
    private $match;
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		$this->view->put_all(array(
            ...
            'JS_DOC' => $this->teams_number ? FootballTournamentService::build_bracket_js_matches($this->compet_id(), $this->teams_number, $this->teams_per_group) : MessageHelper::display('', '')
        ));
	...
	private function build_form()
	{
        ...
        if($this->get_params()->get_looser_bracket())
        {
            ...
            for ($i = $rounds_number; $i >= 1; $i--)
            {
                ...
                for($j = 1; $j <= $matches_number; $j++)
                {
                    ...
                    $match_date = $this->get_match('L', $i, $j)->get_match_date() ? $this->get_match('L', $i, $j)->get_match_date() : new Date();
                    $match_playground = $this->get_match('L', $i, $j)->get_match_playground() ? $this->get_match('L', $i, $j)->get_match_playground() : '';
                    $match_home_id = $this->get_match('L', $i, $j)->get_match_home_id() ? $this->get_match('L', $i, $j)->get_match_home_id() : 0;
                    $match_home_score = $this->get_match('L', $i, $j)->get_match_home_score() ? $this->get_match('L', $i, $j)->get_match_home_score() : '';
                    $match_home_pen = $this->get_match('L', $i, $j)->get_match_home_pen() ? $this->get_match('L', $i, $j)->get_match_home_pen() : '';
                    $match_away_pen = $this->get_match('L', $i, $j)->get_match_away_pen() ? $this->get_match('L', $i, $j)->get_match_away_pen() : '';
                    $match_away_score = $this->get_match('L', $i, $j)->get_match_away_score() ? $this->get_match('L', $i, $j)->get_match_away_score() : '';
                    $match_away_id = $this->get_match('L', $i, $j)->get_match_away_id() ? $this->get_match('L', $i, $j)->get_match_away_id() : 0;
        ...
        for ($i = $rounds_number; $i >= 1; $i--)
        {
            $winner_fieldset->add_field(new FormFieldSpacer('round_' . $i, $this->lang['football.round.' . $i . ''], array('class' => 'form-spacer-big')));
            if ($this->return_matches) {
                if ($i == 1)
                    $matches_number = $this->get_params()->get_looser_bracket() ? $this->teams_number / 4 : $this->round_matches_number($i);
                else
            ...
            for($j = 1; $j <= $matches_number; $j++)
            {
                ...
                $match_date = $this->get_match('W', $i, $j)->get_match_date() ? $this->get_match('W', $i, $j)->get_match_date() : new Date();
                $match_playground = $this->get_match('W', $i, $j)->get_match_playground() ? $this->get_match('W', $i, $j)->get_match_playground() : '';
                $match_home_id = $this->get_match('W', $i, $j)->get_match_home_id() ? $this->get_match('W', $i, $j)->get_match_home_id() : 0;
                $match_home_score = $this->get_match('W', $i, $j)->get_match_home_score() ? $this->get_match('W', $i, $j)->get_match_home_score() : '';
                $match_home_pen = $this->get_match('W', $i, $j)->get_match_home_pen() ? $this->get_match('W', $i, $j)->get_match_home_pen() : '';
                $match_away_pen = $this->get_match('W', $i, $j)->get_match_away_pen() ? $this->get_match('W', $i, $j)->get_match_away_pen() : '';
                $match_away_score = $this->get_match('W', $i, $j)->get_match_away_score() ? $this->get_match('W', $i, $j)->get_match_away_score() : '';
                $match_away_id = $this->get_match('W', $i, $j)->get_match_away_id() ? $this->get_match('W', $i, $j)->get_match_away_id() : 0;
	...
	private function get_match($type, $group, $order)
	{
        ...
        $id = FootballMatchService::get_match_in_group($compet_id, $type, $group, $order) ? FootballMatchService::get_match_in_group($compet_id, $type, $group, $order)->get_id_match() : null;

        if($id !== null)
            try {
                $this->match = FootballMatchService::get_match($id, $type, $group, $order);
	...
	private function generate_response(View $view)
	{
        ...
        $graphical_environment->get_seo_meta_data()->set_canonical_url(FootballUrlBuilder::edit_bracket($compet->get_id_compet()));
        ...
        $breadcrumb->add($this->lang['football.matches.management'], FootballUrlBuilder::edit_bracket($compet->get_id_compet()));
