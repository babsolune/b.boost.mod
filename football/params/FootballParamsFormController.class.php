...
class FootballParamsFormController extends DefaultModuleController
{
	...
	private function build_form()
	{
		...
		if ($this->is_tournament)
		{
            ...
            $tournament_fieldset->add_field(new FormFieldCheckbox('hat_ranking', $this->lang['football.hat.ranking'], $this->get_params()->get_hat_ranking(),
                array(
                    'description' => '<span aria-label="' . $this->lang['football.hat.ranking.clue'] . '"><i class="far fa-circle-question"></i></span>',
                    'events' => array('click' => '
                    if (HTMLForms.getField("hat_ranking").getValue()) {
                        HTMLForms.getField("hat_days").enable();
                        HTMLForms.getField("fill_matches").disable();
                    } else {
                        HTMLForms.getField("hat_days").disable();
                        HTMLForms.getField("fill_matches").enable();
                    }
                ')
                )
            ));
            $tournament_fieldset->add_field(new FormFieldNumberEditor('hat_days', $this->lang['football.hat.days'], $this->get_params()->get_hat_days(),
                array(
                    'description' => $this->lang['football.hat.days.clue'],
                    'min' => 0, 'required' => true,
            ...
            $tournament_fieldset->add_field(new FormFieldCheckbox('fill_matches', $this->lang['football.fill.matches'], $this->get_params()->get_fill_matches(),
                array(
                    'description' => '<span aria-label="' . $this->lang['football.fill.matches.clue'] . '"><i class="far fa-circle-question"></i></span>',
                    'hidden' => $this->get_params()->get_hat_ranking()
                )
            ));
		...
		{
			$penalties_fieldset = new FormFieldsetHTML('penalties', $this->lang['football.params.penalties']);
	...
	private function generate_response(View $view)
	{
		...
		$breadcrumb->add($this->lang['football.params'], FootballUrlBuilder::edit_params($params->get_params_compet_id()));
