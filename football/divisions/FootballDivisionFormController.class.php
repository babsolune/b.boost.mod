...
class FootballDivisionFormController extends DefaultModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
        ...
        $description = $this->lang['football.championship.clue'];
        if ($this->get_division()->get_division_compet_type() == FootballDivision::CHAMPIONSHIP)
            $description = $this->lang['football.championship.clue'];
        elseif ($this->get_division()->get_division_compet_type() == FootballDivision::CUP)
            $description = $this->lang['football.cup.clue'];
        elseif ($this->get_division()->get_division_compet_type() == FootballDivision::TOURNAMENT)
            $description = $this->lang['football.tournament.clue'];
		$fieldset->add_field(new FormFieldSimpleSelectChoice('compet_type', $this->lang['football.compet.type'], $this->get_division()->get_division_compet_type(),
            ...
            array(
                ...
                'events' => array('click' => '
                    console.log(HTMLForms.getField("compet_type").getValue());
                if (HTMLForms.getField("compet_type").getValue() == "'. FootballDivision::CHAMPIONSHIP .'") {
                    jQuery(this).closest(".form-element").find(".field-description").html("' . $this->lang['football.championship.clue'] . '");
                } else if (HTMLForms.getField("compet_type").getValue() == "'. FootballDivision::CUP .'") {
                    jQuery(this).closest(".form-element").find(".field-description").html("' . $this->lang['football.cup.clue'] . '");
                } else if  (HTMLForms.getField("compet_type").getValue() == "'. FootballDivision::TOURNAMENT .'") {
                    jQuery(this).closest(".form-element").find(".field-description").html("' . $this->lang['football.tournament.clue'] . '");
                }
            '))
		));
