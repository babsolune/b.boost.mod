...
class FootballCompetHomeController extends DefaultModuleController
{
    private $compet;

	protected function get_template_to_use()
	{
		return new FileTemplate('football/FootballCompetHomeController.tpl');
	}
	...
	private function count_views_number(HTTPRequestCustom $request)
	{
		...
		{
			if ($request->get_url_referrer() && !TextHelper::strstr($request->get_url_referrer(), FootballUrlBuilder::compet_home($this->compet->get_id_compet())->rel()))
			{
	...
	private function generate_response()
	{
		...
		$breadcrumb->add($this->lang['football.infos']);
