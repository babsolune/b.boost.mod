...
class FootballCalendarService
{
    ...
    public static function build_tournament_calendar(int $compet_id)
    {
        $view = new FileTemplate('football/FootballTournamentCalendarController.tpl');
        ...
        $results = self::$db_querier->select('SELECT matches.*, compet.*
            ...
            ORDER BY matches.match_date ASC, matches.match_playground ASC, matches.match_group ASC, matches.match_order ASC', array(
                'id' => $compet_id
            )
        );
