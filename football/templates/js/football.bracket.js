...
(function($) {
    $.fn.gracket = function(method) {
        ...
        // Defaults
        $.fn.gracket.defaults = {
            gracketClass    : "bracket",
            dateClass       : "game-date",
            matchClass      : "game-nb",
            endClass        : "game-end",
            gameClass       : "game",
            roundClass      : "round",
            roundLabelClass : "round-label",
            teamClass       : "team",
            flagClass       : "flag",
            logoPath        : "",
            nameClass       : "team-name",
            scoreClass      : "team-score",
            winnerClass     : "winner",
            spacerClass     : "game-spacer",
            currentClass    : "team-hightlight",
            seedClass       : "seed",

            cornerRadius    : 3,
            canvasId        : "bracket-canvas",
            canvasClass     : "bracket-canvas",
            canvasLineColor : "var(--txt-main)",
            canvasLineCap   : "round",
            canvasLineWidth : 2,
            canvasLineGap   : 15,
            roundLabels     : [],
        ...
        var
            container = this,
            data = (typeof container.data("gracket") === "undefined") ? [] : (typeof container.data("gracket") === "string") ? JSON.parse(container.data("gracket")) : container.data("gracket"),
            team_count,
            round_count,
            game_count,
            max_round_width = []
        ...
        var methods = {
            init : function(options) {
                ...
                // Calculate wdth of container
                let cell = jQuery('.' + this.gracket.settings.roundClass),
                    bracketWidth = 0;
                cell.each(() => {
                    bracketWidth += cell.outerWidth(true);
                });
                container.css('width', bracketWidth);
            }
        ...
        var helpers = {
            build : {
                team : function(data, node){
                    var flagStyle = (data.logo === '') ? '' : ' style="background-image: url(' + node.logoPath + data.logo + '.png)"';
                    var html = [
                        '<h3>',
                            (typeof data.date === 'undefined') ? '' : '<span class="game-details">',
                                (typeof data.date === 'undefined') ? '' : ((typeof data.match_nb === 'undefined') ? '<em></em>' : '<em class="' + node.matchClass + ' smaller">' +  data.match_nb + '</em>'),
                                (typeof data.date === 'undefined') ? '' : '<em class="' + node.dateClass + ' smaller">' +  data.date + '</em>',
                                (typeof data.date === 'undefined') ? '' : ((typeof data.end_game === 'undefined') ? '<em></em>' : '<em class="' + node.endClass + ' smaller">' +  data.end_game + '</em>'),
                            (typeof data.date === 'undefined') ? '' : '</span>',
                            '<span>',
                                (typeof data.seed === 'undefined') ? '' : '<span class="' + node.seedClass + '">' + ((typeof data.displaySeed === 'undefined') ? ((typeof data.seed === 'undefined') ? '' : data.seed) : data.displaySeed) + '</span>',
                                (typeof data.logo === 'undefined') ? '' : '<span class="' + node.flagClass  + '"' + flagStyle + '>&nbsp;</span>',
                                '<span class="' + node.nameClass  + '">' + data.name + '</span>',
                            '</span>',
                            (typeof data.score === 'undefined') ? '' : '<span class="' + node.scoreClass  + '">',
                                data.score,
                                (typeof data.pen === 'undefined') ? '' : '<sup class="smaller">' + data.pen + '</sup>',
                            '</span>',
                        '</h3>'
                    ].join("");
                    return team = jQuery("<div />", {
                        "html" : html,
