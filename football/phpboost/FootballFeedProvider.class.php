...
class FootballFeedProvider implements FeedProvider
{
	public function get_feeds_list()
	{
		return CategoriesService::get_categories_manager('football')->get_feeds_categories_module()->get_feed_list();
	}
	...
	public function get_feed_data_struct($idcat = 0, $name = '')
	{
		...
		if (CategoriesService::get_categories_manager($module_id)->get_categories_cache()->category_exists($idcat))
		{
			...
			$feed_module_name = LangLoader::get_message('football.module.title', 'common', 'football');
			$data = new FeedData();
			$data->set_title($feed_module_name . ' - ' . $site_name);
			$data->set_date(new Date());
			$data->set_link(SyndicationUrlBuilder::rss('football', $idcat));
			$data->set_host(HOST);
			...
			$now = new Date();
			$results = $querier->select('SELECT football.id, football.id_category, football.title, football.rewrited_title, football.content, football.update_date, football.thumbnail, cat.rewrited_name AS rewrited_name_cat
				FROM ' . FootballSetup::$football_compet_table . ' football
				LEFT JOIN '. FootballSetup::$football_cats_table .' cat ON cat.id = football.id_category
				WHERE football.id_category IN :ids_categories
				AND (published = 1 OR (published = 2 AND publishing_start_date < :timestamp_now AND (publishing_end_date > :timestamp_now OR publishing_end_date = 0)))
				ORDER BY football.update_date DESC', array(
			...
			foreach ($results as $row)
			{
				...
				$link = FootballUrlBuilder::compet_home($row['id']);
