<?php
/*##################################################
 *                      ModcatfullDisplayCategoryController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class ModcatfullDisplayCategoryController extends ModuleController
{
	private $lang;
	private $config;
	...
	private function init()
	{
		...
		$this->config = ModcatfullConfig::load();
		$this->comments_config = new ModcatfullComments();
		$this->notation_config = new ModcatfullNotation();
	}
	...
	private function build_view()
	{
		...
		$mode = $request->get_getstring('sort', ModcatfullUrlBuilder::DEFAULT_SORT_MODE);
		$field = $request->get_getstring('field', ModcatfullUrlBuilder::DEFAULT_SORT_FIELD);
	...
	private function build_items_listing_view(Date $now, $field, $mode, $page, $subcategories_page)
	{
		...
		$result = PersistenceContext::get_querier()->select('SELECT modcatfull.*, member.*, com.number_comments, notes.average_notes, notes.number_notes, note.note
		...
		$this->view->put_all(array(
			'C_MOSAIC' => $this->config->get_display_type() == ModcatfullConfig::MOSAIC_DISPLAY,
			'C_LIST' => $this->config->get_display_type() == ModcatfullConfig::LIST_DISPLAY,
			'C_TABLE' => $this->config->get_display_type() == ModcatfullConfig::TABLE_DISPLAY,
			...
			'C_MODERATION' => ModcatfullAuthorizationsService::check_authorizations($this->get_category()->get_id())->moderation(),
			...
			'CATEGORY_ID' => $this->get_category()->get_id(),
			'U_EDIT_CATEGORY' => $this->get_category()->get_id() == Category::ROOT_CATEGORY ? ModcatfullUrlBuilder::configuration()->rel() : ModcatfullUrlBuilder::edit_category($this->get_category()->get_id())->rel()
		));
	...
	{
		$subcategories = ModcatfullService::get_categories_manager()->get_categories_cache()->get_children($this->get_category()->get_id(), ModcatfullService::get_authorized_categories($this->get_category()->get_id()));
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('events' => array('change' => 'document.location = "'. ModcatfullUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_category()
	{
		if ($this->category === null)
		{
			...
			{
				try {
					$this->category = ModcatfullService::get_categories_manager()->get_categories_cache()->get_category($id);
			...
			{
				$this->category = ModcatfullService::get_categories_manager()->get_categories_cache()->get_category(Category::ROOT_CATEGORY);
			}
		}
	...
	{
		$number_items = PersistenceContext::get_querier()->count(ModcatfullSetup::$modcatfull_table, $condition, $parameters);

		$pagination = new ModulePagination($page, $number_items, (int)ModcatfullConfig::load()->get_items_number_per_page());
		$pagination->set_url(ModcatfullUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), $field, $mode, '%d', $subcategories_page));
	...
	private function get_subcategories_pagination($subcategories_number, $categories_number_per_page, $field, $mode, $page, $subcategories_page)
	{
		...
		$pagination->set_url(ModcatfullUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), $field, $mode, $page, '%d'));
	...
	private function check_authorizations()
	{
		...
		{
			if (!ModcatfullAuthorizationsService::check_authorizations($this->get_category()->get_id())->read())
			{
				$error_controller = PHPBoostErrors::user_not_authorized();
				DispatchManager::redirect($error_controller);
			}
	...
	private function generate_response()
	{
		...
		if ($this->category->get_id() != Category::ROOT_CATEGORY)
			$graphical_environment->set_page_title($this->category->get_name(), $this->lang['modcatfull.module.title']);
		else
			$graphical_environment->set_page_title($this->lang['modcatfull.module.title']);

		$graphical_environment->get_seo_meta_data()->set_description($this->category->get_description());
		$graphical_environment->get_seo_meta_data()->set_canonical_url(ModcatfullUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['modcatfull.module.title'], ModcatfullUrlBuilder::home());

		$categories = array_reverse(ModcatfullService::get_categories_manager()->get_parents($this->category->get_id(), true));
		foreach ($categories as $id => $category)
		{
			if ($category->get_id() != Category::ROOT_CATEGORY)
				$breadcrumb->add($category->get_name(), ModcatfullUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));
		}
