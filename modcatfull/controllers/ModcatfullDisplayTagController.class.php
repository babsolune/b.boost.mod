<?php
/*##################################################
 *		       ModcatfullDisplayTagController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class ModcatfullDisplayTagController extends ModuleController
{
	private $lang;
	private $view;
	private $keyword;
	...
	private function get_keyword()
	{
		if ($this->keyword === null)
		{
			...
			{
				try {
					$this->keyword = ModcatfullService::get_keywords_manager()->get_keyword('WHERE rewrited_name=:rewrited_name', array('rewrited_name' => $rewrited_name));
	...
	private function build_view($request)
	{
		...
		LEFT JOIN ' . DB_TABLE_KEYWORDS_RELATIONS . ' relation ON relation.module_id = \'modcatfull\' AND relation.id_in_module = modcatfull.id
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('events' => array('change' => 'document.location = "'. ModcatfullUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_pagination($condition, $parameters, $field, $mode, $page)
	{
		...
		$pagination = new ModulePagination($page, $result['nbr_items'], ModcatfullConfig::load()->get_items_number_per_page());
		$pagination->set_url(ModcatfullUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $field, $mode, '%d'));
	...
	{
		if (!(ModcatfullAuthorizationsService::check_authorizations()->read()))
		{
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($this->get_keyword()->get_name(), $this->lang['modcatfull.module.title']);
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['modcatfull.seo.description.tag'], array('subject' => $this->get_keyword()->get_name())));
		$graphical_environment->get_seo_meta_data()->set_canonical_url(ModcatfullUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'),AppContext::get_request()->get_getint('page', 1)));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['modcatfull.module.title'], ModcatfullUrlBuilder::home());
		$breadcrumb->add($this->get_keyword()->get_name(), ModcatfullUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'),AppContext::get_request()->get_getint('page', 1)));
