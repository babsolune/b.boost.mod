...
class Itemcatfull
{
	...
	const DISABLE_LINKS_VISIBILITY = 0;
	const ENABLE_LINKS_VISIBILITY = 1;

	const DEFAULT_PICTURE = '/modcatfull/templates/images/default.png';
	...
	{
		return ModcatfullService::get_categories_manager()->get_categories_cache()->get_category($this->category_id);
	}
	...
	public function get_real_description()
	{
		...
		else
		{
			...
			return TextHelper::cut_string(@strip_tags($clean_contents[0], '<br><br/>'), (int)ModcatfullConfig::load()->get_characters_number_to_cut());
		}
	}
	...
	public function get_links_visibility()
	{
		return $this->links_visibility;
	}

	public function set_links_visibility($links_visibility)
	{
		$this->links_visibility = $links_visibility;
	}
	...
	{
		$now = new Date();
		return ModcatfullAuthorizationsService::check_authorizations($this->category_id)->read() && ($this->get_publication_state() == self::PUBLISHED_NOW || ($this->get_publication_state() == self::PUBLICATION_DATE && $this->get_publication_start_date()->is_anterior_to($now) && ($this->enabled_end_date ? $this->get_publication_end_date()->is_posterior_to($now) : true)));
	}
	...
	public function get_keywords()
	{
		...
		{
			$this->keywords = ModcatfullService::get_keywords_manager()->get_keywords($this->id);
		}
	...
	{
		return ModcatfullAuthorizationsService::check_authorizations($this->category_id)->write() || ModcatfullAuthorizationsService::check_authorizations($this->category_id)->contribution();
	}

	public function is_authorized_to_edit()
	{
		return ModcatfullAuthorizationsService::check_authorizations($this->category_id)->moderation() || ((ModcatfullAuthorizationsService::check_authorizations($this->get_category_id())->write() || (ModcatfullAuthorizationsService::check_authorizations($this->get_category_id())->contribution() && !$this->is_published())) && $this->get_author_user()->get_id() == AppContext::get_current_user()->get_id() && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL));
	}
	...
	public function get_properties()
	{
		return array(
			...
			'links_visibility'       => $this->get_links_visibility(),
	...
	public function set_properties(array $properties)
	{
		...
		$this->set_website_url(new Url($properties['website_url']));
		$this->set_visits_number($properties['visits_number']);
		$this->set_file_url(new Url($properties['file_url']));
		$this->set_file_size($properties['file_size']);
		$this->set_downloads_number($properties['downloads_number']);
		$this->set_links_visibility($properties['links_visibility']);
		...
		$notation->set_module_name('modcatfull');
		...
		$this->formated_size = (float)number_format($this->file_size / pow(1024, $power), 2, '.', ',') . ' ' . $units[$power];
	}
	...
	public function init_default_properties($category_id = Category::ROOT_CATEGORY)
	{
		...
		$this->links_visibility = self::ENABLE_LINKS_VISIBILITY;
	...
	public function get_array_tpl_vars()
	{
		...
		$new_content        = new ModcatfullNewContent();
		$notation_config    = new ModcatfullNotation();

		if(!filter_var($this->get_file_url()->rel(), FILTER_VALIDATE_URL, FILTER_FLAG_PATH_REQUIRED))
			$file_size = $this->formated_size;
		else
			$file_size = $this->lang['modcatfull.external.file'];

		if ($this->get_links_visibility() == self::DISABLE_LINKS_VISIBILITY)
			$visible_links = AppContext::get_current_user()->check_level(User::MEMBER_LEVEL);
		else
			$visible_links = AppContext::get_current_user()->check_level(User::VISITOR_LEVEL);
		...
		return array_merge(
			...
			'C_READ_MORE'                      => !$this->get_description_enabled() && TextHelper::strlen($contents) > ModcatfullConfig::load()->get_characters_number_to_cut() && $description != @strip_tags($contents, '<br><br/>'),
			'C_SOURCES'                        => $nbr_sources > 0,
			'C_CAROUSEL'                       => $nbr_pictures > 0,
			'C_VISIBLE_LINKS'				   => $visible_links,
			'C_WEBLINK'						   => !empty($this->get_website_url()->rel()),
			'C_FILE'						   => !empty($this->get_file_url()->rel()),
			'C_USER_CONNECTED'				   => AppContext::get_current_user()->check_level(User::MEMBER_LEVEL),
			'C_DEADLINK'					   => !empty($this->get_website_url()->rel()) || !empty($this->get_file_url()->rel()),
			'C_FILE_SIZE' 					   => !empty($this->get_file_size()),
			...
			'L_COMMENTS'         => CommentsService::get_number_and_lang_comments('modcatfull', $this->get_id()),
			'COMMENTS_NUMBER'    => CommentsService::get_number_comments('modcatfull', $this->get_id()),
			'VIEWS_NUMBER'       => $this->get_views_number(),
			'VISITS_NUMBER'      => $this->get_visits_number(),
			'FILE_SIZE' 		 => $file_size,
			'DOWNLOADS_NUMBER'   => $this->get_downloads_number(),
			...
			'U_EDIT_CATEGORY'      => $category->get_id() == Category::ROOT_CATEGORY ? ModcatfullUrlBuilder::configuration()->rel() : ModcatfullUrlBuilder::edit_category($category->get_id())->rel(),

			//Links
			'U_COMMENTS'    => ModcatfullUrlBuilder::display_items_comments($category->get_id(), $category->get_rewrited_name(), $this->get_id(), $this->get_rewrited_title())->rel(),
			'U_AUTHOR'      => UserUrlBuilder  ::profile($this->get_author_user()->get_id())->rel(),
			'U_CATEGORY'    => ModcatfullUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name())->rel(),
			'U_ITEM'        => ModcatfullUrlBuilder::display_item($category->get_id(), $category->get_rewrited_name(), $this->get_id(), $this->get_rewrited_title())->rel(),
			'U_WEBLINK' 	=> ModcatfullUrlBuilder::visit_item($this->id)->rel(),
			'U_FILE' 		=> ModcatfullUrlBuilder::download_item($this->id)->rel(),
			'U_EDIT_ITEM'   => ModcatfullUrlBuilder::edit_item($this->id, AppContext::get_request()->get_getint('page', 1))->rel(),
			'U_DELETE_ITEM' => ModcatfullUrlBuilder::delete_item($this->id)->rel(),
			'U_DEADLINK' 	=> ModcatfullUrlBuilder::dead_link($this->id)->rel(),
			'U_SYNDICATION' => ModcatfullUrlBuilder::category_syndication($category->get_id())->rel(),
			'U_PRINT_ITEM'  => ModcatfullUrlBuilder::print_item($this->get_id(), $this->get_rewrited_title())->rel()
			)
		);
	}
}
