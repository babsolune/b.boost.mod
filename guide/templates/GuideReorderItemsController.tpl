# IF C_ITEMS #
	<script>
		var GuideItems = function(id){
			this.id = id;
			this.items_number = {ITEMS_NUMBER};
		};

		GuideItems.prototype = {
		...
		var GuideItems = new GuideItems('items-list');
		jQuery(document).ready(function() {
			GuideItems.init_sortable();
		});
	</script>
# ENDIF #
...
<section id="module-guide">
	<header class="section-header">
		<div class="controls align-right">
			<a class="offload" href="${relative_url(SyndicationUrlBuilder::rss('guide', CATEGORY_ID))}" aria-label="{@common.syndication}"><i class="fa fa-rss warning" aria-hidden="true"></i></a>
	...
	# IF C_ITEMS #
		<div class="sub-section">
			<div class="content-container">
				<div class="content">
					<form action="{REWRITED_SCRIPT}" method="post" id="position-update-form" onsubmit="GuideItems.serialize_sortable();" class="guide-reorder-form">
							...
							<ul id="items-list" class="sortable-block">
								# START items #
									<li class="sortable-element# IF items.C_NEW_CONTENT # new-content# ENDIF #" id="list-{items.ID}" data-id="{items.ID}">
										...
										<script>
											jQuery(document).ready(function() {
												var guide_question = new GuideItem({items.ID}, GuideItems);
