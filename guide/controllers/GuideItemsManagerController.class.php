...
class GuideItemsManagerController extends DefaultModuleController
{
	...
	private function build_table()
	{
		...
		$table_model = new SQLHTMLTableModel(GuideSetup::$guide_items_table, 'items-manager', $columns, new HTMLTableSortingRule('creation_date', HTMLTableSortingRule::DESC));

		$table_model->set_layout_title($this->lang['guide.items.management']);
		$table_model->add_permanent_filter('c.active_content = 1');

		$table_model->set_filters_menu_title($this->lang['guide.filter.items']);
		...
		foreach ($result as $row)
		{
			...
			$edit_link = new EditLinkHTMLElement(GuideUrlBuilder::edit($item->get_id()));
			$delete_link = new DeleteLinkHTMLElement(GuideUrlBuilder::delete($item->get_id(), 0));

			$user_group_color = User::get_group_color($user->get_groups(), $user->get_level(), true);
			$author = $user->get_id() !== User::VISITOR_LEVEL ? new LinkHTMLElement(UserUrlBuilder::profile($user->get_id()), $user->get_display_name(), (!empty($user_group_color) ? array('style' => 'color: ' . $user_group_color) : array()), UserService::get_level_class($user->get_level())) : $user->get_display_name();

			$row = array(
				new HTMLTableRowCell(new LinkHTMLElement(GuideUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), $item->get_item_content()->get_title()), 'left'),
				new HTMLTableRowCell(new LinkHTMLElement(GuideUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name()), ($category->get_id() == Category::ROOT_CATEGORY ? $this->lang['common.none.alt'] : $category->get_name()))),
        ...
        if ($request->get_string('delete-selected-elements', false))
        {
            for ($i = 1; $i <= $this->elements_number; $i++)
            {
                if ($request->get_value('delete-checkbox-' . $i, 'off') == 'on')
                {
                    if (isset($this->ids[$i]))
                    {
						$item = GuideService::get_item($this->ids[$i]);
                        GuideService::delete($this->ids[$i], 0);
            ...
            AppContext::get_response()->redirect(GuideUrlBuilder::manage(), $this->lang['warning.process.success']);
        }
    }

	private function check_authorizations()
	{
		if (!GuideAuthorizationsService::check_authorizations()->moderation())
		{
