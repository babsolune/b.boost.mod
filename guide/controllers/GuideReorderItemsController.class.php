...
class GuideReorderItemsController extends DefaultModuleController
{
	private $category;

	protected function get_template_to_use()
	{
		return new FileTemplate('guide/GuideReorderItemsController.tpl');
	}
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		if ($request->get_value('submit', false))
		{
			...
			AppContext::get_response()->redirect(GuideUrlBuilder::display_category($this->get_category()->get_id(), $this->get_category()->get_rewrited_name()), $this->lang['warning.success.position.update']);
		}
	...
	private function update_position(HTTPRequestCustom $request)
	{
		...
		{
			GuideService::update_position($tree->id, $position);
		}
	}
	...
	private function generate_response()
	{
		...
		if (empty($description))
			$description = StringVars::replace_vars($this->lang['guide.seo.description.root'], array('site' => GeneralConfig::load()->get_site_name())) . ($this->get_category()->get_id() != Category::ROOT_CATEGORY ? ' ' . $this->lang['category.category'] . ' ' . $this->get_category()->get_name() : '') . ' ' . $this->lang['guide.questions.reorder'];
		...
		$breadcrumb->add($this->lang['items.reorder'], GuideUrlBuilder::reorder_items($this->get_category()->get_id(), $this->get_category()->get_rewrited_name()));
