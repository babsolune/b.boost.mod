...
class GuideItemFormController extends DefaultModuleController
{
	public function execute(HTTPRequestCustom $request)
	{
		...
		{
			$this->form->get_field_by_id('custom_level')->set_hidden($item_content->get_content_level() != GuideItemContent::CUSTOM_LEVEL);
			$this->save();
			$this->redirect();
		}
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		$form->set_layout_title($this->get_item()->get_id() === null ? $this->lang['guide.add.item'] : ($this->lang['guide.edit.item']));

		$fieldset = new FormFieldsetHTML('guide', $this->lang['form.parameters']);
		$form->add_fieldset($fieldset);
		if ($this->get_item()->get_id() !== null)
			$fieldset->set_description($this->lang['guide.message.draft']);
		...
		$fieldset->add_field(new FormFieldCheckbox('summary_enabled', $this->lang['form.enable.summary'], $item_content->is_summary_enabled(),
			array(
				'description' => StringVars::replace_vars($this->lang['form.summary.clue'], array('number' => GuideConfig::load()->get_auto_cut_characters_number())),
				'events' => array('click' => '
		...
		$options_fieldset->add_field(new FormFieldSimpleSelectChoice('content_level', $this->lang['guide.level'], $item_content->get_content_level(),
			array(
				new FormFieldSelectChoiceOption('', GuideItemContent::NO_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['guide.level.wip'], GuideItemContent::WIP_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['guide.level.sketch'], GuideItemContent::SKETCH_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['guide.level.redo'], GuideItemContent::REDO_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['guide.level.claim'], GuideItemContent::CLAIM_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['guide.level.trust'], GuideItemContent::TRUST_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['guide.level.custom'], GuideItemContent::CUSTOM_LEVEL),
			),
			array(
				'events' => array('change' => '
					if (HTMLForms.getField("content_level").getValue() == \'' . GuideItemContent::CUSTOM_LEVEL . '\') {
		...
		$options_fieldset->add_field(new FormFieldThumbnail('thumbnail', $this->lang['form.thumbnail'], $item_content->get_thumbnail()->relative(), GuideItem::THUMBNAIL_URL));
		...
		if(!$this->is_new_item)
		{
			$options_fieldset->add_field(new FormFieldMultiLineTextEditor('change_reason', $this->lang['guide.change.reason'], '',
				array('rows' => 5)
			));
		}
		...
		if (GuideAuthorizationsService::check_authorizations($this->get_item()->get_id_category())->moderation())
		{
			...
			if (!$this->get_item()->is_published())
			{
				$publication_fieldset->add_field(new FormFieldCheckbox('update_creation_date', $this->lang['form.update.creation.date'], false,
					array('hidden' => $this->get_item()->get_publishing_state() != GuideItem::NOT_PUBLISHED)
				));
			}

			$publication_fieldset->add_field(new FormFieldSimpleSelectChoice('published', $this->lang['form.publication'], $this->get_item()->get_id() === null ? $this->get_item()->get_publishing_state() : GuideItem::NOT_PUBLISHED,
				array(
					new FormFieldSelectChoiceOption($this->lang['form.publication.draft'], GuideItem::NOT_PUBLISHED),
					new FormFieldSelectChoiceOption($this->lang['form.publication.now'], GuideItem::PUBLISHED),
			...
			$publication_fieldset->add_field($publishing_start_date = new FormFieldDateTime('publishing_start_date', $this->lang['form.start.date'], ($this->get_item()->get_publishing_start_date() === null ? new Date() : $this->get_item()->get_publishing_start_date()),
				array('hidden' => ($request->is_post_method() ? ($request->get_postint(__CLASS__ . '_publication_state', 0) != GuideItem::DEFERRED_PUBLICATION) : ($this->get_item()->get_publishing_state() != GuideItem::DEFERRED_PUBLICATION)))
			));
			...
			$publication_fieldset->add_field(new FormFieldCheckbox('end_date_enabled', $this->lang['form.enable.end.date'], $this->get_item()->is_end_date_enabled(),
				array(
					'hidden' => ($request->is_post_method() ? ($request->get_postint(__CLASS__ . '_publication_state', 0) != GuideItem::DEFERRED_PUBLICATION) : ($this->get_item()->get_publishing_state() != GuideItem::DEFERRED_PUBLICATION)),
					'events' => array('click' => '
						if (HTMLForms.getField("end_date_enabled").getValue()) {
							HTMLForms.getField("publishing_end_date").enable();
						} else {
	...
	{
		return (!GuideAuthorizationsService::check_authorizations()->write() && GuideAuthorizationsService::check_authorizations()->contribution());
	}
	...
	private function save()
	{
		...
		if ($this->form->get_value('content_level')->get_raw_value() == GuideItemContent::CUSTOM_LEVEL)
		...
		if (!GuideAuthorizationsService::check_authorizations($item->get_id_category())->moderation())
		{
			...
			if (GuideAuthorizationsService::check_authorizations($item->get_id_category())->contribution() && !GuideAuthorizationsService::check_authorizations($item->get_id_category())->write())
				$item->set_publishing_state(GuideItem::NOT_PUBLISHED);
		}
		else
		{
			...
			if ($item->get_publishing_state() == GuideItem::DEFERRED_PUBLICATION)
			{
	...
	private function contribution_actions(GuideItem $item)
	{
		$item_content = $this->get_item()->get_item_content();
		if ($this->is_contributor_member())
		{
			...
			$contribution->set_fixing_url(GuideUrlBuilder::edit($item->get_id())->relative());
			$contribution->set_poster_id(AppContext::get_current_user()->get_id());
			$contribution->set_module('guide');
		...
		{
			$corresponding_contributions = ContributionService::find_by_criteria('guide', $item->get_id());
	...
	private function redirect()
	{
		...
		{
			if ($this->is_new_item)
				AppContext::get_response()->redirect(GuideUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), StringVars::replace_vars($this->lang['guide.message.success.add'], array('title' => $item_content->get_title())));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : GuideUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title())), StringVars::replace_vars($this->lang['guide.message.success.edit'], array('title' => $item_content->get_title())));
		}
		else
		{
			if ($this->is_new_item)
				AppContext::get_response()->redirect(GuideUrlBuilder::display_pending(), StringVars::replace_vars($this->lang['guide.message.success.add'], array('title' => $item_content->get_title())));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : GuideUrlBuilder::display_pending()), StringVars::replace_vars($this->lang['guide.message.success.edit'], array('title' => $item_content->get_title())));
		}
	}
	...
	private function generate_response(View $view)
	{
		...
		$location_id = $item->get_id() ? 'guide-edit-'. $item->get_id() : '';
		...
		else
		{
			...
			$breadcrumb->add($this->lang['guide.edit.item'], GuideUrlBuilder::edit($item->get_id()));
		}
