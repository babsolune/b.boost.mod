...
class ClubsDeadLinkController extends AbstractController
{
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		if ($this->item !== null && (!ClubsAuthorizationsService::check_authorizations($this->item->get_id_category())->read() || !ClubsAuthorizationsService::check_authorizations()->display_download_link()))
		{
		...
		{
			if (!PersistenceContext::get_querier()->row_exists(PREFIX . 'events', 'WHERE id_in_module=:id_in_module AND module=\'clubs\' AND current_status = 0', array('id_in_module' => $this->item->get_id())))
			{
				...
				$contribution->set_fixing_url(ClubsUrlBuilder::edit($this->item->get_id())->relative());
