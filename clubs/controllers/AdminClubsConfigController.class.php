...
class AdminClubsConfigController extends AdminModuleController
{
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		{
			$this->save();
			$this->form->get_field_by_id('items_per_row')->set_hidden($this->config->get_display_type() !== ClubsConfig::GRID_VIEW);
	...
	private function build_form()
	{
        ...
        $fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->form_lang['form.display.type'], $this->config->get_display_type(),
			array(
				new FormFieldSelectChoiceOption($this->form_lang['form.display.type.grid'], ClubsConfig::GRID_VIEW, array('data_option_icon' => 'fa fa-th-large')),
				new FormFieldSelectChoiceOption($this->form_lang['form.display.type.table'], ClubsConfig::TABLE_VIEW, array('data_option_icon' => 'fa fa-table'))
			),
			array(
				'select_to_list' => true,
				'events' => array('change' => '
					if (HTMLForms.getField("display_type").getValue() == \'' . ClubsConfig::GRID_VIEW . '\') {
        ...
        $fieldset->add_field(new FormFieldNumberEditor('items_per_row', $this->form_lang['form.items.per.row'], $this->config->get_items_per_row(),
			array(
				...
				'hidden' => $this->config->get_display_type() !== ClubsConfig::GRID_VIEW
			),
		...
		$fieldset->add_field(new FormFieldNumberEditor('categories_per_row', $this->form_lang['form.categories.per.row'], $this->config->get_items_per_row(),
	...
	private function save()
	{
		...
		if ($this->form->get_value('display_type')->get_raw_value() == ClubsConfig::GRID_VIEW)
