<section id="module-modcatart">
	<header>
		<h1>
			<a href="${relative_url(SyndicationUrlBuilder::rss('modcatart', CATEGORY_ID))}" title="${LangLoader::get_message('syndication', 'common')}"><i class="fa fa-syndication"></i></a>
			# IF C_PENDING #{@modcatart.pending.items}# ELSE #{@modcatart.module.title}# IF NOT C_ROOT_CATEGORY # - {CATEGORY_NAME}# ENDIF ## ENDIF # # IF C_CATEGORY ## IF IS_ADMIN #<a href="{U_EDIT_CATEGORY}" title="${LangLoader::get_message('edit', 'common')}"><i class="fa fa-edit smaller"></i></a># ENDIF ## ENDIF #
		</h1>
	...
	# IF C_SUB_CATEGORIES #
	<div class="subcat-container elements-container# IF C_SEVERAL_CATS_COLUMNS # columns-{CATS_COLUMNS_NUMBER}# ENDIF #">
		# START sub_categories #
		<div class="subcat-element block">
			<div class="subcat-content">
				# IF C_DISPLAY_CAT_ICONS #
				# IF sub_categories.C_CATEGORY_IMAGE #<a itemprop="about" href="{sub_categories.U_CATEGORY}"><img itemprop="thumbnailUrl" src="{sub_categories.CATEGORY_IMAGE}" alt="{sub_categories.CATEGORY_NAME}" /></a># ENDIF #
				<br />
				# ENDIF #
				<a itemprop="about" href="{sub_categories.U_CATEGORY}">{sub_categories.CATEGORY_NAME}</a>
				<br />
				<span class="small">{sub_categories.ITEMS_NUMBER} # IF sub_categories.C_MORE_THAN_ONE_ITEM #${TextHelper::lcfirst(LangLoader::get_message('modcatart.items', 'common', 'modcatart'))}# ELSE #${TextHelper::lcfirst(LangLoader::get_message('modcatart.item', 'common', 'modcatart'))}# ENDIF #</span>
			</div>
		</div>
		# END sub_categories #
	</div>
	...
	# ELSE #
		...
		# ELSE #
			...
			<div class="elements-container# IF C_SEVERAL_COLUMNS # columns-{COLUMNS_NUMBER}# ENDIF#">
				# START items #
					<article id="modcatart-items-{items.ID}" class="modcatart-items several-items# IF C_MOSAIC # block# ENDIF ## IF items.C_NEW_CONTENT # new-content# ENDIF #" itemscope="itemscope" itemtype="http://schema.org/CreativeWork">
						<header>
							<h2>
						...
						<div class="spacer"></div>
						<aside>
						<div id="modcatart-sources-container">
