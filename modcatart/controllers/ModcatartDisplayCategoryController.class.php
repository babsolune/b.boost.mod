<?php
/*##################################################
 *                      ModcatartDisplayCategoryController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class ModcatartDisplayCategoryController extends ModuleController
{
	private $lang;
	private $config;
	...
	private function init()
	{
		...
		$this->config = ModcatartConfig::load();
		$this->comments_config = new ModcatartComments();
		$this->notation_config = new ModcatartNotation();
	}
	...
	private function build_view()
	{
		...
		$mode = $request->get_getstring('sort', ModcatartUrlBuilder::DEFAULT_SORT_MODE);
		$field = $request->get_getstring('field', ModcatartUrlBuilder::DEFAULT_SORT_FIELD);
	...
	private function build_items_listing_view(Date $now, $field, $mode, $page, $subcategories_page)
	{
		...
		$result = PersistenceContext::get_querier()->select('SELECT modcatart.*, member.*, com.number_comments, notes.average_notes, notes.number_notes, note.note
		...
		$this->view->put_all(array(
			'C_MOSAIC' => $this->config->get_display_type() == ModcatartConfig::MOSAIC_DISPLAY,
			'C_LIST' => $this->config->get_display_type() == ModcatartConfig::LIST_DISPLAY,
			'C_TABLE' => $this->config->get_display_type() == ModcatartConfig::TABLE_DISPLAY,
			...
			'C_MODERATION' => ModcatartAuthorizationsService::check_authorizations($this->get_category()->get_id())->moderation(),
	...
	{
		$subcategories = ModcatartService::get_categories_manager()->get_categories_cache()->get_children($this->get_category()->get_id(), ModcatartService::get_authorized_categories($this->get_category()->get_id()));
		...
		foreach ($subcategories as $id => $category)
		{
			...
			if ($cats_displayed_nbr > $subcategories_pagination->get_display_from() && $cats_displayed_nbr <= ($subcategories_pagination->get_display_from() + $subcategories_pagination->get_number_items_per_page()))
			{
				...
				$this->view->assign_block_vars('sub_categories', array(
					...
					'U_CATEGORY' => ModcatartUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name())->rel()
				));
			}
		}
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('events' => array('change' => 'document.location = "'. ModcatartUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_category()
	{
		if ($this->category === null)
		{
			...
			{
				try {
					$this->category = ModcatartService::get_categories_manager()->get_categories_cache()->get_category($id);
			...
			{
				$this->category = ModcatartService::get_categories_manager()->get_categories_cache()->get_category(Category::ROOT_CATEGORY);
			}
		}
	...
	{
		$number_items = PersistenceContext::get_querier()->count(ModcatartSetup::$modcatart_table, $condition, $parameters);

		$pagination = new ModulePagination($page, $number_items, (int)ModcatartConfig::load()->get_items_number_per_page());
		$pagination->set_url(ModcatartUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), $field, $mode, '%d', $subcategories_page));
	...
	private function get_subcategories_pagination($subcategories_number, $categories_number_per_page, $field, $mode, $page, $subcategories_page)
	{
		...
		$pagination->set_url(ModcatartUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), $field, $mode, $page, '%d'));
	...
	private function check_authorizations()
	{
		...
		{
			if (!ModcatartAuthorizationsService::check_authorizations($this->get_category()->get_id())->read())
			{
				$error_controller = PHPBoostErrors::user_not_authorized();
				DispatchManager::redirect($error_controller);
			}
	...
	private function generate_response()
	{
		...
		if ($this->category->get_id() != Category::ROOT_CATEGORY)
			$graphical_environment->set_page_title($this->category->get_name(), $this->lang['modcatart.module.title']);
		else
			$graphical_environment->set_page_title($this->lang['modcatart.module.title']);

		$graphical_environment->get_seo_meta_data()->set_description($this->category->get_description());
		$graphical_environment->get_seo_meta_data()->set_canonical_url(ModcatartUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['modcatart.module.title'], ModcatartUrlBuilder::home());

		$categories = array_reverse(ModcatartService::get_categories_manager()->get_parents($this->category->get_id(), true));
		foreach ($categories as $id => $category)
		{
			if ($category->get_id() != Category::ROOT_CATEGORY)
				$breadcrumb->add($category->get_name(), ModcatartUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));
		}
