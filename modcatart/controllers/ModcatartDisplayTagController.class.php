<?php
/*##################################################
 *		       ModcatartDisplayTagController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class ModcatartDisplayTagController extends ModuleController
{
	private $lang;
	private $view;
	private $keyword;
	...
	private function get_keyword()
	{
		if ($this->keyword === null)
		{
			...
			{
				try {
					$this->keyword = ModcatartService::get_keywords_manager()->get_keyword('WHERE rewrited_name=:rewrited_name', array('rewrited_name' => $rewrited_name));
	...
	private function build_view($request)
	{
		...
		LEFT JOIN ' . DB_TABLE_KEYWORDS_RELATIONS . ' relation ON relation.module_id = \'modcatart\' AND relation.id_in_module = modcatart.id
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('events' => array('change' => 'document.location = "'. ModcatartUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_pagination($condition, $parameters, $field, $mode, $page)
	{
		...
		$pagination = new ModulePagination($page, $result['nbr_items'], ModcatartConfig::load()->get_items_number_per_page());
		$pagination->set_url(ModcatartUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $field, $mode, '%d'));
	...
	{
		if (!(ModcatartAuthorizationsService::check_authorizations()->read()))
		{
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($this->get_keyword()->get_name(), $this->lang['modcatart.module.title']);
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['modcatart.seo.description.tag'], array('subject' => $this->get_keyword()->get_name())));
		$graphical_environment->get_seo_meta_data()->set_canonical_url(ModcatartUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'),AppContext::get_request()->get_getint('page', 1)));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['modcatart.module.title'], ModcatartUrlBuilder::home());
		$breadcrumb->add($this->get_keyword()->get_name(), ModcatartUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'),AppContext::get_request()->get_getint('page', 1)));
