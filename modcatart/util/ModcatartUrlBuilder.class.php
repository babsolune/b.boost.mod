 ...
 */
class ModcatartUrlBuilder
{
	const DEFAULT_SORT_FIELD = 'date';
	const DEFAULT_SORT_MODE = 'desc';

	private static $dispatcher = '/modcatart';
	...
	{
		return SyndicationUrlBuilder::rss('modcatart', $id);
	}
	 ...
	 * @return Url
	 */
	public static function print_item($id_itemcatart, $rewrited_title)
	{
		return DispatchManager::get_url(self::$dispatcher, '/print/' . $id_itemcatart . '-' .$rewrited_title . '/');
	}
	 ...
	 * @return Url
	 */
	public static function display_item($category_id, $rewrited_name_category, $id_itemcatart, $rewrited_title, $page = 1)
	{
		$page = $page !== 1 ? $page . '/' : '';
		return DispatchManager::get_url(self::$dispatcher, '/' . $category_id . '-' . $rewrited_name_category . '/' . $id_itemcatart . '-' .$rewrited_title . '/' . $page);
	}

	/**
	 * @return Url
	 */
	public static function display_items_comments($category_id, $rewrited_name_category, $id_itemcatart, $rewrited_title)
	{
		return DispatchManager::get_url(self::$dispatcher, '/' . $category_id . '-' . $rewrited_name_category . '/' . $id_itemcatart . '-' . $rewrited_title . '/#comments-list');
	}
