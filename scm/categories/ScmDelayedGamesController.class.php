...
class ScmDelayedGamesController extends DefaultModuleController
{
	...
	{
		return new FileTemplate('scm/main/ScmDelayedGamesController.tpl');
	}
	...
	private function build_view(HTTPRequestCustom $request)
	{
        ...
        foreach($games as $game)
        {
            ...
            if($game['game_date'] <= $now->get_timestamp() && !$item->is_exempted_game() && !$item->is_forfeit_game() && !$item->has_score())
        ...
        foreach ($matchdays as $matchday => $dates)
        {
            ...
            foreach ($dates as $game)
            {
                ...
                $event = ScmEventService::get_event($item->get_game_event_id());
                switch ($event->get_event_type()) {
                    case ScmEvent::CHAMPIONSHIP :
                        $u_edit = ScmUrlBuilder::edit_days_games($event->get_id(), $event->get_event_slug(), $item->get_game_cluster())->rel();
                        $round = $this->lang['scm.day'];
                    break;
                    case ScmEvent::TOURNAMENT :
                        $u_edit = ScmUrlBuilder::edit_groups_games($event->get_id(), $event->get_event_slug(), $item->get_game_cluster())->rel();
                        $round = $this->lang['scm.round'];
                    break;
                    case ScmEvent::CUP :
                        $u_edit = ScmUrlBuilder::edit_brackets_games($event->get_id(), $event->get_event_slug(), $item->get_game_cluster())->rel();
                        $round = $this->lang['scm.round'];
                    break;
                    case ScmEvent::PRACTICE :
                        $u_edit = ScmUrlBuilder::edit_practice_games($event->get_id(), $event->get_event_slug(), $item->get_game_cluster())->rel();
                        $round = $this->lang['scm.day'];
                    break;
                }
