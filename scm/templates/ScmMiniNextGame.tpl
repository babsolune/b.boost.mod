# IF C_ITEMS #
	<div class="cell-table">
		<table id="table-mini-scm" class="table">
			<thead>
				<tr>
					<th aria-label="# IF C_SORT_BY_DATE #{@common.sort.by.date}# ELSE #{@scm.ranking}# ENDIF #">
						<i class="fa# IF C_SORT_BY_DATE #r fa-calendar-alt# ELSE # fa-trophy# ENDIF # hidden-small-screens" aria-hidden="true"></i>
						<span class="hidden-large-screens"># IF C_SORT_BY_DATE #{@common.sort.by.date}# ELSE #{@scm.ranking}# ENDIF #</span>
					</th>
					<th>{@common.name}</th>
			...
			<tbody>
					# START items #
						<tr class="category-{items.CATEGORY_ID}">
							...
							<td# IF C_SORT_BY_NOTATION # class="mini-scm-table-name"# ENDIF #>
								<a class="offload" href="{items.U_EVENT}">
									{items.TITLE}
								</a>
