...
class ScmEventHomeService
{
    ...
    public static function build_championship_home(int $event_id)
    {
        ...
        $view->put_all([
            ...
            'PREV_GAMES' => ScmGameFormat::format_cluster(ScmGameService::get_games_in_cluster($event_id, $prev_day)),
            'NEXT_GAMES' => ScmGameFormat::format_cluster(ScmGameService::get_games_in_cluster($event_id, $next_day)),
            'PREV_DAY' => $prev_day,
            'NEXT_DAY' => $next_day,
        ]);
        ...
        foreach ($groups as $k => $group)
        {
            ...
            foreach ($group as $team)
            {
                ...
                $view->assign_block_vars('team_groups.teams', [
                    ...
                    'U_TEAM_CALENDAR' => ScmUrlBuilder::display_team_calendar($event_id, ScmEventService::get_event_slug($event_id), $team['id_team'])->rel(),
    ...
    public static function build_practice_home(int $event_id)
    {
        ...
        $view->put_all([
            'C_HAS_GAMES' => ScmGameService::has_games($event_id),
            'GAMES_LIST' => ScmGameFormat::format_cluster($games),
        ]);
        return $view;
    }
}
