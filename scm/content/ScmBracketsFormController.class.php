...
class ScmBracketsFormController extends DefaultModuleController
{
	...
	private function generate_response(View $view)
	{
        ...
        $graphical_environment->set_page_title($this->lang['scm.brackets.management'], $event->get_event_name() . ($category->get_id() != Category::ROOT_CATEGORY ? ' - ' . $category->get_name() : '') . ' - ' . $this->lang['scm.module.title'] . ' - ' . GeneralConfig::load()->get_site_name());
        $graphical_environment->get_seo_meta_data()->set_description($this->lang['scm.brackets.management']);
		...
		$breadcrumb->add($this->lang['scm.brackets.management'], ScmUrlBuilder::edit_brackets($event->get_id(), $event->get_event_slug()));
