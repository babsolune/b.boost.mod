...
class ScmRankingService
{
    ...
    public static function init_rankings($event_id)
    {
        if (count(self::get_rankings($event_id)) == 0)
            ScmRankingService::add_ranking($event_id);
    }

    /** Create a new entry in the database table. */
	public static function add_ranking($event_id)
	{
		$result = self::$db_querier->insert(ScmSetup::$scm_ranking_table, ['ranking_event_id' => $event_id, 'content' => '']);
		return $result->get_last_inserted_id();
	}

    public static function delete_ranking(int $event_id)
	{
		self::$db_querier->delete(ScmSetup::$scm_ranking_table, 'WHERE ranking_event_id = :ranking_event_id', ['ranking_event_id' => $event_id]);
    }

    /** Get all rankings from an event */
	public static function get_rankings(int $event_id):array
	{
	...
	public static function general_groups_ranking($event_id, $cluster)
    {
        $game_teams = self::build_groups_teams($event_id, $cluster);
        $results = self::build_results($event_id, $game_teams);
        $final_ranks = self::sort_general_ranks($results, $event_id);
	...
	public static function general_groups_finals_ranking($event_id, $cluster)
    {
        $game_teams = self::build_groups_finals_teams($event_id, $cluster);
        $results = self::build_results($event_id, $game_teams);
        $final_ranks = self::sort_general_ranks($results, $event_id);
	...
	public static function build_results($event_id, $game_teams)
    {
        ...
        for($i = 0; $i < count($game_teams); $i++)
        {
            ...
            if ($game_teams[$i]['team_id'])
                $teams[] = [
                    'cluster'       => $game_teams[$i]['cluster'],
                    'round'         => $game_teams[$i]['round'],
	...
	public static function build_groups_teams($event_id, $cluster)
    {
        $games = $days_games = [];
        $games[] = ScmGameService::get_games_in_cluster($event_id, $cluster);
        ...
        foreach ($team_games as $game)
        {
            ...
            if ($game['game_home_score'] == '' && $game['game_away_score'] == '')
            {
                ...
                elseif (ScmTeamService::get_team($game['game_home_id'])->get_team_status() == ScmParams::EXEMPT || ScmTeamService::get_team($game['game_away_id'])->get_team_status() == ScmParams::EXEMPT)
                    ...
                    $team_form[] = ['result' => 'unplayed', 'class' => 'link-color', 'score' => ''];
            }
        }
        return $team_form;
    }
