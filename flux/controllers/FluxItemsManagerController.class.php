...
class FluxItemsManagerController extends AdminModuleController
{
	private $lang;
	private $view;
	...
	private function build_table()
	{
		...
		foreach ($result as $row)
		{
			...
			$edit_item = new LinkHTMLElement(FluxUrlBuilder::edit($item->get_id()), '', array('title' => LangLoader::get_message('common.edit', 'common-lang')), 'fa fa-edit');
			$delete_item = new LinkHTMLElement(FluxUrlBuilder::delete($item->get_id()), '', array('title' => LangLoader::get_message('common.delete', 'common-lang'), 'data-confirmation' => 'delete-element'), 'far fa-trash-alt');

			$user_group_color = User::get_group_color($user->get_groups(), $user->get_level(), true);
			$author = $user->get_id() !== User::VISITOR_LEVEL ? new LinkHTMLElement(UserUrlBuilder::profile($user->get_id()), $user->get_display_name(), (!empty($user_group_color) ? array('style' => 'color: ' . $user_group_color) : array()), UserService::get_level_class($user->get_level())) : $user->get_display_name();

			$row = array(
				new HTMLTableRowCell(new LinkHTMLElement(FluxUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), $item->get_title()), 'left'),
				new HTMLTableRowCell(new LinkHTMLElement(
					...
					array('data-color-surround' => $category->get_id() != Category::ROOT_CATEGORY ? $category->get_color() : FluxConfig::load()->get_default_color()), 'pinned')
				),
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($this->lang['flux.management'], $this->lang['flux.module.title']);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(FluxUrlBuilder::manage());

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['flux.module.title'], FluxUrlBuilder::home());

		$breadcrumb->add($this->lang['flux.management'], FluxUrlBuilder::manage());
