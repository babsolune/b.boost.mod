<section id="module-modlist">
	<header>
		<h1>
			...
			{@modlist.module.title}# IF NOT C_ROOT_CATEGORY # - {CATEGORY_NAME}# ENDIF # # IF IS_ADMIN #<a href="{U_EDIT_CATEGORY}" title="${LangLoader::get_message('edit', 'common')}"><i class="fa fa-edit smaller"></i></a># ENDIF #
		</h1>
	</header>
	<nav>
		<ul>
			# START category_list #
				<li data-parent="{category_list.PARENT_ID}">
					<a href="{category_list.U_CATEGORY}">{category_list.NAME}</a>
				</li>
			# END category_list #
		</ul>
	</nav>
	# INCLUDE NOT_VISIBLE_MESSAGE #
	<article itemscope="itemscope" itemtype="http://schema.org/Itemlist" id="article-modlist-{ID}" class="article-modlist# IF C_NEW_CONTENT # new-content# ENDIF #">
		<header>
			<h2>
			...
			<div class="more">
				...
				&nbsp;<i class="fa fa-eye" title="{VIEWS_NUMBER} {@modlist.sort.field.views}"></i>&nbsp;<span title="{VIEWS_NUMBER} {@modlist.sort.field.views}">{VIEWS_NUMBER}</span>
