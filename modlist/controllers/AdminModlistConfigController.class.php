<?php
/*##################################################
 *                   AdminModlistConfigController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class AdminModlistConfigController extends AdminModuleController
{
	/**
	 * @var HTMLForm
	 */
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		return new AdminModlistDisplayResponse($tpl, $this->lang['module.config.title']);
	}
	...
	private function build_form()
	{
		...
		$fieldset = new FormFieldsetHTML('modlist.configuration', LangLoader::get_message('configuration', 'admin-common'));
		...
		$fieldset->add_field(new FormFieldCheckbox('display_icon_cats', $this->lang['modlist.configuration.cats.icon.display'], $this->config->are_cat_icons_enabled()
		));

		$fieldset->add_field(new FormFieldCheckbox('display_sort_filters', $this->lang['modlist.configuration.sort.filter.display'], $this->config->are_sort_filters_enabled()
		));

		$fieldset->add_field(new FormFieldCheckbox('enabled_items_suggestions', $this->lang['modlist.configuration.suggestions.display'], $this->config->get_enabled_items_suggestions(),
		...
		$fieldset->add_field(new FormFieldNumberEditor('suggested_items_nb', $this->lang['modlist.configuration.suggestions.nb'], $this->config->get_suggested_items_nb(),
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->lang['modlist.configuration.display.type'], $this->config->get_display_type(),
			array(
				new FormFieldSelectChoiceOption($this->lang['modlist.configuration.mosaic.type.display'], ModlistConfig::MOSAIC_DISPLAY),
				new FormFieldSelectChoiceOption($this->lang['modlist.configuration.list.type.display'], ModlistConfig::LIST_DISPLAY),
				new FormFieldSelectChoiceOption($this->lang['modlist.configuration.table.type.display'], ModlistConfig::TABLE_DISPLAY)
			)
		));

		$fieldset->add_field(new FormFieldNumberEditor('characters_number_to_cut', $this->lang['modlist.configuration.characters.number.to.cut'], $this->config->get_characters_number_to_cut(),
			array('min' => 20, 'max' => 1000, 'required' => true),
			array(new FormFieldConstraintIntegerRange(20, 1000))
		));

		$fieldset->add_field(new FormFieldCheckbox('display_descriptions_to_guests', $this->lang['modlist.configuration.display.descriptions.to.guests'], $this->config->are_descriptions_displayed_to_guests()));
	...
	private function save()
	{
		...
		ModlistConfig::save();
		ModlistService::get_categories_manager()->regenerate_cache();
	}
}
?>
