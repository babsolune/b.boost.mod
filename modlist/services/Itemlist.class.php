...
class Itemlist
{
	...
	{
		return ModlistService::get_categories_manager()->get_categories_cache()->get_category($this->category_id);
	}
	...
	{
		$now = new Date();
		return ModlistAuthorizationsService::check_authorizations($this->category_id)->read() && ($this->get_publication_state() == self::PUBLISHED_NOW || ($this->get_publication_state() == self::PUBLICATION_DATE && $this->get_publication_start_date()->is_anterior_to($now) && ($this->enabled_end_date ? $this->get_publication_end_date()->is_posterior_to($now) : true)));
	}
	...
	{
		return ModlistAuthorizationsService::check_authorizations($this->category_id)->write() || ModlistAuthorizationsService::check_authorizations($this->category_id)->contribution();
	}

	public function is_authorized_to_edit()
	{
		return ModlistAuthorizationsService::check_authorizations($this->category_id)->moderation() || ((ModlistAuthorizationsService::check_authorizations($this->get_category_id())->write() || (ModlistAuthorizationsService::check_authorizations($this->get_category_id())->contribution() && !$this->is_published())) && $this->get_author_user()->get_id() == AppContext::get_current_user()->get_id() && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL));
	}
