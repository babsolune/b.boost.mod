...
class ModmixConfig extends AbstractConfigData
{
	const ITEMS_NUMBER_PER_PAGE = 'items_number_per_page';
	const ITEMS_NUMBER_PER_CATEGORY = 'items_number_per_category';
	...
	public function get_items_number_per_category()
	{
		return $this->get_property(self::ITEMS_NUMBER_PER_CATEGORY);
	}

	public function set_items_number_per_category($number)
	{
		$this->set_property(self::ITEMS_NUMBER_PER_PAGE, $number);
	}
	...
	public function get_default_values()
	{
		return array(
			...
			self::ITEMS_NUMBER_PER_CATEGORY => 10,
			...
			self::ROOT_CATEGORY_DESCRIPTION => LangLoader::get_message('root_category_description', 'config', 'modmix'),
	...
	{
		return ConfigManager::load(__CLASS__, 'modmix', 'config');
	}
	...
	{
		ConfigManager::save('modmix', self::load(), 'config');
	}
}
?>
