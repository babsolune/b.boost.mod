<?php
/*##################################################
 *                      ModmixDisplayCategoryController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class ModmixDisplayCategoryController extends ModuleController
{
	private $lang;
	private $config;
	...
	private function build_view()
	{
		...
		$mode = $request->get_getstring('sort', ModmixUrlBuilder::DEFAULT_SORT_MODE);
		$field = $request->get_getstring('field', ModmixUrlBuilder::DEFAULT_SORT_FIELD);
	...
	{
		$subcategories = ModmixService::get_categories_manager()->get_categories_cache()->get_children($this->get_category()->get_id(), ModmixService::get_authorized_categories($this->get_category()->get_id()));
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('events' => array('change' => 'document.location = "'. ModmixUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_subcategories_pagination($subcategories_number, $categories_number_per_page, $field, $mode, $page, $subcategories_page)
	{
		...
		$pagination->set_url(ModmixUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), $field, $mode, $page, '%d'));
