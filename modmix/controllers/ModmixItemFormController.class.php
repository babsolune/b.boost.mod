...
class ModmixItemFormController extends ModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		if (ModmixAuthorizationsService::check_authorizations($this->get_itemmix()->get_category_id())->moderation())
		{
		...
		if (ModmixService::get_categories_manager()->get_categories_cache()->has_categories())
		{
			...
			$fieldset->add_field(ModmixService::get_categories_manager()->get_select_categories_form_field('category_id', $this->common_lang['form.category'], $this->get_itemmix()->get_category_id(), $search_category_children_options));
		}
	...
	private function get_itemmix()
	{
		if ($this->itemmix === null)
		{
			...
			else
			{
				...
				$this->itemmix->init_default_properties(AppContext::get_request()->get_getint('category_id', Category::ROOT_CATEGORY));
			}
		}
		return $this->itemmix;
	}
	...
	private function save()
	{
		...
		if (ModmixService::get_categories_manager()->get_categories_cache()->has_categories())
			$itemmix->set_category_id($this->form->get_value('category_id')->get_raw_value());
		...
		if (!ModmixAuthorizationsService::check_authorizations($itemmix->get_category_id())->moderation())
		{
			if ($itemmix->get_id() === null)
			...
			if (ModmixAuthorizationsService::check_authorizations($itemmix->get_category_id())->contribution() && !ModmixAuthorizationsService::check_authorizations($itemmix->get_category_id())->write())
			...
			if ($this->is_contributor_member())
			{
				...
				$contribution->set_auth(
					Authorizations::capture_and_shift_bit_auth(
						ModmixService::get_categories_manager()->get_heritated_authorizations($itemmix->get_category_id(), Category::MODERATION_AUTHORIZATIONS, Authorizations::AUTH_CHILD_PRIORITY),
	...
	private function generate_response(View $tpl)
	{
		...
		if ($itemmix->get_id() === null)
		{
			$breadcrumb->add($this->lang['modmix.add'], ModmixUrlBuilder::add_item($itemmix->get_category_id()));
			...
			$graphical_environment->get_seo_meta_data()->set_canonical_url(ModmixUrlBuilder::add_item($itemmix->get_category_id()));
		}
		else
		{
			$categories = array_reverse(ModmixService::get_categories_manager()->get_parents($itemmix->get_category_id(), true));
