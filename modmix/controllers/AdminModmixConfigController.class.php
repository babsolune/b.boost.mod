<?php
/*##################################################
 *                   AdminModmixConfigController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class AdminModmixConfigController extends AdminModuleController
{
	/**
	 * @var HTMLForm
	 */
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		return new AdminModmixDisplayResponse($tpl, $this->lang['module.config.title']);
	}
	...
	private function build_form()
	{
		...
		$fieldset = new FormFieldsetHTML('modmix.configuration', LangLoader::get_message('configuration', 'admin-common'));
		...
		$fieldset->add_field(new FormFieldNumberEditor('items_number_per_category', $this->lang['modmix.items.number.per.category'], $this->config->get_items_number_per_category(),
			array('min' => 1, 'max' => 50, 'required' => true),
			array(new FormFieldConstraintIntegerRange(1, 50))
		));
		...
		$fieldset->add_field(new FormFieldCheckbox('display_icon_cats', $this->lang['modmix.configuration.cats.icon.display'], $this->config->are_cat_icons_enabled()
		));

		$fieldset->add_field(new FormFieldCheckbox('display_sort_filters', $this->lang['modmix.configuration.sort.filter.display'], $this->config->are_sort_filters_enabled()
		));

		$fieldset->add_field(new FormFieldCheckbox('enabled_items_suggestions', $this->lang['modmix.configuration.suggestions.display'], $this->config->get_enabled_items_suggestions(),
		...
		$fieldset->add_field(new FormFieldNumberEditor('suggested_items_nb', $this->lang['modmix.configuration.suggestions.nb'], $this->config->get_suggested_items_nb(),
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->lang['modmix.configuration.display.type'], $this->config->get_display_type(),
			array(
				new FormFieldSelectChoiceOption($this->lang['modmix.configuration.mosaic.type.display'], ModmixConfig::MOSAIC_DISPLAY),
				new FormFieldSelectChoiceOption($this->lang['modmix.configuration.list.type.display'], ModmixConfig::LIST_DISPLAY),
				new FormFieldSelectChoiceOption($this->lang['modmix.configuration.table.type.display'], ModmixConfig::TABLE_DISPLAY)
			)
		));

		$fieldset->add_field(new FormFieldNumberEditor('characters_number_to_cut', $this->lang['modmix.configuration.characters.number.to.cut'], $this->config->get_characters_number_to_cut(),
			array('min' => 20, 'max' => 1000, 'required' => true),
			array(new FormFieldConstraintIntegerRange(20, 1000))
		));

		$fieldset->add_field(new FormFieldCheckbox('display_descriptions_to_guests', $this->lang['modmix.configuration.display.descriptions.to.guests'], $this->config->are_descriptions_displayed_to_guests()));
	...
	private function save()
	{
		...
		$this->config->set_items_number_per_category($this->form->get_value('items_number_per_category'));
		...
		ModmixConfig::save();
		ModmixService::get_categories_manager()->regenerate_cache();
	}
}
?>
