<?php
/*##################################################
 *		       ModmixDisplayTagController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class ModmixDisplayTagController extends ModuleController
{
	private $lang;
	private $view;
	private $keyword;
	...
	private function init()
	{
		...
		$this->view = new FileTemplate('modmix/ModmixDisplayCategoryController.tpl');
	...
	private function get_keyword()
	{
		if ($this->keyword === null)
		{
			...
			{
				try {
					$this->keyword = ModmixService::get_keywords_manager()->get_keyword('WHERE rewrited_name=:rewrited_name', array('rewrited_name' => $rewrited_name));
	...
	private function build_view($request)
	{
		...
		$result = PersistenceContext::get_querier()->select('SELECT modmix.*, member.*, com.number_comments, notes.number_notes, notes.average_notes, note.note
		FROM ' . ModmixSetup::$modmix_table . ' modmix
		LEFT JOIN ' . DB_TABLE_KEYWORDS_RELATIONS . ' relation ON relation.module_id = \'modmix\' AND relation.id_in_module = modmix.id
		...
		while ($row = $result->fetch())
		{
			...
			$this->build_keywords_view($itemmix);

			$this->view->assign_block_vars('items', $itemmix->get_array_tpl_vars());
			$this->build_sources_view($itemmix);
		}
		$result->dispose();
	}
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('events' => array('change' => 'document.location = "'. ModmixUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_pagination($condition, $parameters, $field, $mode, $page)
	{
		...
		$pagination = new ModulePagination($page, $result['nbr_items'], ModmixConfig::load()->get_items_number_per_page());
		$pagination->set_url(ModmixUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $field, $mode, '%d'));
	...
	{
		if (!(ModmixAuthorizationsService::check_authorizations()->read()))
		{
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($this->get_keyword()->get_name(), $this->lang['modmix.module.title']);
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['modmix.seo.description.tag'], array('subject' => $this->get_keyword()->get_name())));
		$graphical_environment->get_seo_meta_data()->set_canonical_url(ModmixUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'),AppContext::get_request()->get_getint('page', 1)));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['modmix.module.title'], ModmixUrlBuilder::home());
		$breadcrumb->add($this->get_keyword()->get_name(), ModmixUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'),AppContext::get_request()->get_getint('page', 1)));
