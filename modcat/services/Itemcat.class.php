...
class Itemcat
{
	...
	const DEFAULT_PICTURE = '/modcat/templates/images/default.png';
	...
	{
		return ModcatService::get_categories_manager()->get_categories_cache()->get_category($this->category_id);
	}
	...
	public function get_real_description()
	{
		...
		else
		{
			...
			return TextHelper::cut_string(@strip_tags($clean_contents[0], '<br><br/>'), (int)ModcatConfig::load()->get_characters_number_to_cut());
		}
	}
	...
	{
		$now = new Date();
		return ModcatAuthorizationsService::check_authorizations($this->category_id)->read() && ($this->get_publication_state() == self::PUBLISHED_NOW || ($this->get_publication_state() == self::PUBLICATION_DATE && $this->get_publication_start_date()->is_anterior_to($now) && ($this->enabled_end_date ? $this->get_publication_end_date()->is_posterior_to($now) : true)));
	}
	...
	public function get_keywords()
	{
		...
		{
			$this->keywords = ModcatService::get_keywords_manager()->get_keywords($this->id);
		}
	...
	{
		return ModcatAuthorizationsService::check_authorizations($this->category_id)->write() || ModcatAuthorizationsService::check_authorizations($this->category_id)->contribution();
	}

	public function is_authorized_to_edit()
	{
		return ModcatAuthorizationsService::check_authorizations($this->category_id)->moderation() || ((ModcatAuthorizationsService::check_authorizations($this->get_category_id())->write() || (ModcatAuthorizationsService::check_authorizations($this->get_category_id())->contribution() && !$this->is_published())) && $this->get_author_user()->get_id() == AppContext::get_current_user()->get_id() && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL));
	}
	...
	public function set_properties(array $properties)
	{
		...
		$notation->set_module_name('modcat');
	...
	public function get_array_tpl_vars()
	{
		...
		$new_content        = new ModcatNewContent();
		$notation_config    = new ModcatNotation();
		...
		return array_merge(
			...
			'C_READ_MORE'                      => !$this->get_description_enabled() && TextHelper::strlen($contents) > ModcatConfig::load()->get_characters_number_to_cut() && $description != @strip_tags($contents, '<br><br/>'),
			...
			'L_COMMENTS'         => CommentsService::get_number_and_lang_comments('modcat', $this->get_id()),
			'COMMENTS_NUMBER'    => CommentsService::get_number_comments('modcat', $this->get_id()),
			'VIEWS_NUMBER'       => $this->get_views_number(),
			...
			'U_EDIT_CATEGORY'      => $category->get_id() == Category::ROOT_CATEGORY ? ModcatUrlBuilder::configuration()->rel() : ModcatUrlBuilder::edit_category($category->get_id())->rel(),

			//Links
			'U_COMMENTS'    => ModcatUrlBuilder::display_items_comments($category->get_id(), $category->get_rewrited_name(), $this->get_id(), $this->get_rewrited_title())->rel(),
			...
			'U_ITEM'        => ModcatUrlBuilder::display_item($category->get_id(), $category->get_rewrited_name(), $this->get_id(), $this->get_rewrited_title())->rel(),
			'U_EDIT_ITEM'   => ModcatUrlBuilder::edit_item($this->id, AppContext::get_request()->get_getint('page', 1))->rel(),
			'U_DELETE_ITEM' => ModcatUrlBuilder::delete_item($this->id)->rel(),
			'U_SYNDICATION' => ModcatUrlBuilder::category_syndication($category->get_id())->rel(),
			'U_PRINT_ITEM'  => ModcatUrlBuilder::print_item($this->get_id(), $this->get_rewrited_title())->rel()
			)
		);
	}
}
