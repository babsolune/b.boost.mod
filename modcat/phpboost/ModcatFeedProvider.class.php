...
class ModcatFeedProvider implements FeedProvider
{
	...
	public function get_feed_data_struct($idcat = 0, $name = '')
	{
		if (ModcatService::get_categories_manager()->get_categories_cache()->category_exists($idcat))
		{
			...
			$now = new Date();
			$results = $querier->select('SELECT modcat.id, modcat.category_id, modcat.title, modcat.rewrited_title, modcat.thumbnail_url,
			...
			WHERE modcat.category_id IN :cats_ids
			...
			foreach ($results as $row)
			{
				...
				$link = ModcatUrlBuilder::display_item($row['category_id'], $row['rewrited_name_cat'], $row['id'], $row['rewrited_title']);
				$item = new FeedItem();
				...
				$item->set_auth(ModcatService::get_categories_manager()->get_heritated_authorizations($row['category_id'], Category::READ_AUTHORIZATIONS, Authorizations::AUTH_PARENT_PRIORITY));
				$data->add_item($item);
			}
			$results->dispose();
