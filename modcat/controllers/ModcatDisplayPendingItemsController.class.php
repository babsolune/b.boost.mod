<?php
/*##################################################
 *		    ModcatDisplayPendingItemsController.class.php
 *                            -------------------
 *   begin                : Month XX, 2017
...
class ModcatDisplayPendingItemsController extends ModuleController
{
	private $lang;
	private $view;
	private $form;
	...
	private function build_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, array(
			...
			), array('events' => array('change' => 'document.location = "'. ModcatUrlBuilder::display_pending_items()->rel() . '" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function build_view($request)
	{
		...
		' . (!ModcatAuthorizationsService::check_authorizations()->moderation() ? ' AND author_user_id = :user_id' : '') . '
		...
		$this->view->put_all(array(
			'C_PENDING' => true,
			'C_MOSAIC' => $config->get_display_type() == ModcatConfig::MOSAIC_DISPLAY,
	...
	{
		if (!(ModcatAuthorizationsService::check_authorizations()->write() || ModcatAuthorizationsService::check_authorizations()->contribution() || ModcatAuthorizationsService::check_authorizations()->moderation()))
		{
	...
	{
		$items_number = PersistenceContext::get_querier()->count(ModcatSetup::$modcat_table, $condition, $parameters);

		$pagination = new ModulePagination($page, $items_number, (int)ModcatConfig::load()->get_items_number_per_page());
		$pagination->set_url(ModcatUrlBuilder::display_pending_items($field, $mode, '/%d'));
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($this->lang['modcat.pending.items'], $this->lang['modcat.module.title']);
		$graphical_environment->get_seo_meta_data()->set_description($this->lang['modcat.seo.description.pending']);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(ModcatUrlBuilder::display_pending_items(AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['modcat.module.title'], ModcatUrlBuilder::home());
		$breadcrumb->add($this->lang['modcat.pending.items'], ModcatUrlBuilder::display_pending_items(AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));
