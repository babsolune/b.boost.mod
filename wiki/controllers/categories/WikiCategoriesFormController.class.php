...
class WikiCategoriesFormController extends DefaultCategoriesFormController
{
	protected function build_form(HTTPRequestCustom $request)
	{
		self::$lang = LangLoader::get_all_langs('wiki');
		...
		$fieldset->add_field(new FormFieldThumbnail('thumbnail', self::$lang['form.thumbnail'], $this->get_category()->get_thumbnail()->relative(), WikiCategory::THUMBNAIL_URL,
			array()
		));
		...
		$auth_settings = new AuthorizationsSettings(array_merge(RootCategory::get_authorizations_settings(), array(
			new VisitorDisabledActionAuthorization(self::$lang['wiki.manage.archives'], WikiAuthorizationsService::MANAGE_ARCHIVES_AUTHORIZATIONS)
		)));
