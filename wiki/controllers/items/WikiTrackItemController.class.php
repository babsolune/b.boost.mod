...
class WikiTrackItemController extends DefaultModuleController
{
	public function execute(HTTPRequestCustom $request)
	{
		...
		if (!empty($item_id))
		{
			...
			if (!in_array($current_user_id, array_keys(WikiService::get_tracked_items($item_id))) && !in_array($item_id, array_keys(WikiService::get_tracked_items($item_id))))
			{
				WikiService::track_item($item_id, $current_user_id);
