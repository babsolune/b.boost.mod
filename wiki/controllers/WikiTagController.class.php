...
class WikiTagController extends DefaultModuleController
{
	private $keyword;
	...
	public function build_view(HTTPRequestCustom $request)
	{
		...
		LEFT JOIN ' . DB_TABLE_KEYWORDS_RELATIONS . ' relation ON relation.module_id = \'wiki\' AND relation.id_in_module = i.id
	...
	private function get_pagination($condition, $parameters, $page)
	{
		...
		$pagination = new ModulePagination($page, $result['items_number'], (int)WikiConfig::load()->get_items_per_page());
	...
	private function generate_response(HTTPRequestCustom $request)
	{
		...
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['wiki.seo.description.tag'], array('subject' => $this->get_keyword()->get_name())), $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(WikiUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $page));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->config->get_module_name(), WikiUrlBuilder::home());
		$breadcrumb->add($this->get_keyword()->get_name(), WikiUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $page));
