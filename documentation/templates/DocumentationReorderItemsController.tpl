# IF C_ITEMS #
	<script>
		...
		var DocumentationItem = function(id, documentation_items){
			this.id = id;
			this.DocumentationItems = documentation_items;

			if (DocumentationItems.items_number > 1)
				DocumentationItems.change_reposition_pictures();
		};

		var DocumentationItems = new DocumentationItems('items-list');
		jQuery(document).ready(function() {
			DocumentationItems.init_sortable();
		});
	</script>
# ENDIF #
...
<section id="module-documentation">
	<header class="section-header">
		<div class="controls align-right">
			<a class="offload" href="${relative_url(SyndicationUrlBuilder::rss('documentation', CATEGORY_ID))}" aria-label="{@common.syndication}"><i class="fa fa-rss warning" aria-hidden="true"></i></a>
	...
	# IF C_ITEMS #
		<div class="sub-section">
			<div class="content-container">
				<div class="content">
					<form action="{REWRITED_SCRIPT}" method="post" id="position-update-form" onsubmit="DocumentationItems.serialize_sortable();" class="documentation-reorder-form">
							...
							<ul id="items-list" class="sortable-block">
								# START items #
									<li class="sortable-element# IF items.C_NEW_CONTENT # new-content# ENDIF #" id="list-{items.ID}" data-id="{items.ID}">
										...
										<script>
											jQuery(document).ready(function() {
												var documentation_question = new DocumentationItem({items.ID}, DocumentationItems);
