<section id="module-documentation" class="several-items">
	<header class="section-header">
		...
		<h1>
			...
			# ELSE #
				# IF C_MEMBER_ITEMS #
					# IF C_MY_ITEMS #{@documentation.my.items}# ELSE #{@documentation.member.items} {MEMBER_NAME}# ENDIF #
				# ELSE #
					# IF C_ROOT_CATEGORY #{@documentation.module.title}# ELSE ## IF C_TAG_ITEMS #<span class="smaller">{@common.keyword}: </span># ENDIF #{CATEGORY_NAME}# ENDIF #
				# ENDIF #
			# ENDIF #
		</h1>
	</header>
							...
							<ul class="items-list-{categories.CATEGORY_ID}">
								# IF C_CONTROLS #
									# IF categories.C_SEVERAL_ITEMS #
										<a class="offload reorder-items" class="reorder-items" href="{categories.U_REORDER_ITEMS}" aria-label="{@items.reorder}"><i class="fa fa-fw fa-exchange-alt"></i></a>
									# ENDIF #
								# ENDIF #
				...
				<div class="content-container">
					...
					# ELSE #
						<div class="cell-flex # IF C_GRID_VIEW #cell-columns-{ITEMS_PER_ROW}# ELSE #cell-row# ENDIF #">
							# START items #
								<article id="documentation-item-{items.ID}" class="documentation-item category-{items.CATEGORY_ID} cell# IF items.C_NEW_CONTENT # new-content# ENDIF #" itemscope="itemscope" itemtype="https://schema.org/CreativeWork">
