...
class DocumentationCategoryController extends DefaultModuleController
{
	...
	{
		$now = new Date();
		$categories = CategoriesService::get_categories_manager('documentation')->get_categories_cache()->get_categories();
		$authorized_categories = CategoriesService::get_authorized_categories(Category::ROOT_CATEGORY, true, 'documentation');
		...
		foreach ($categories as $id => $category) {
			if (in_array($id, $authorized_categories)) {
				$this->view->assign_block_vars('categories', array(
					...
					'U_CATEGORY'         => DocumentationUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name(), 'documentation')->rel(),
					'U_REORDER_ITEMS'    => DocumentationUrlBuilder::reorder_items($category->get_id(), $category->get_rewrited_name())->rel(),
				));
	...
	private function build_view(HTTPRequestCustom $request)
	{
		...
		$subcategories = CategoriesService::get_categories_manager('documentation')->get_categories_cache()->get_children($this->get_category()->get_id(), CategoriesService::get_authorized_categories($this->get_category()->get_id(), $this->config->is_summary_displayed_to_guests(), 'documentation'));
		...
		$this->view->put_all(array(
			...
			'C_CONTROLS'                 => DocumentationAuthorizationsService::check_authorizations($this->get_category()->get_id())->moderation(),
			...
			'C_DISPLAY_REORDER_LINK'     => $result->get_rows_count() > 1 && DocumentationAuthorizationsService::check_authorizations($this->get_category()->get_id())->moderation(),
			...
			'U_EDIT_CATEGORY'      => $this->get_category()->get_id() == Category::ROOT_CATEGORY ? DocumentationUrlBuilder::configuration()->rel() : CategoriesUrlBuilder::edit($this->get_category()->get_id(), 'documentation')->rel(),
			'U_REORDER_ITEMS'      => DocumentationUrlBuilder::reorder_items($this->get_category()->get_id(), $this->get_category()->get_rewrited_name())->rel(),
		));
	...
	private function get_pagination($condition, $parameters, $page, $subcategories_page)
	{
		...
		$pagination->set_url(DocumentationUrlBuilder::display_category($this->get_category()->get_id(), $this->get_category()->get_rewrited_name(), '%d', $subcategories_page));
	...
	private function get_subcategories_pagination($subcategories_number, $categories_per_page, $page, $subcategories_page)
	{
		...
		$pagination->set_url(DocumentationUrlBuilder::display_category($this->get_category()->get_id(), $this->get_category()->get_rewrited_name(), $page, '%d'));
	...
	private function get_category()
	{
		if ($this->category === null)
		{
			...
			{
				try {
					$this->category = CategoriesService::get_categories_manager('documentation')->get_categories_cache()->get_category($id);
			...
			{
				$this->category = CategoriesService::get_categories_manager('documentation')->get_categories_cache()->get_category(Category::ROOT_CATEGORY);
			}
		}
	...
	private function check_authorizations()
	{
		...
		{
			if (!DocumentationAuthorizationsService::check_authorizations($this->get_category()->get_id())->read())
			{
				$error_controller = PHPBoostErrors::user_not_authorized();
				DispatchManager::redirect($error_controller);
			}
	...
	private function generate_response(HTTPRequestCustom $request)
	{
		...
		if ($this->get_category()->get_id() != Category::ROOT_CATEGORY)
			$graphical_environment->set_page_title($this->get_category()->get_name(), $this->lang['documentation.module.title'], $page);
		else
			$graphical_environment->set_page_title($this->lang['documentation.module.title'], '', $page);

		$description = $this->get_category()->get_description();
		if (empty($description))
			$description = StringVars::replace_vars($this->lang['documentation.seo.description.root'], array('site' => GeneralConfig::load()->get_site_name())) . ($this->get_category()->get_id() != Category::ROOT_CATEGORY ? ' ' . $this->lang['category.category'] . ' ' . $this->get_category()->get_name() : '');
		$graphical_environment->get_seo_meta_data()->set_description($description, $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(DocumentationUrlBuilder::display_category($this->get_category()->get_id(), $this->get_category()->get_rewrited_name(), $page));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['documentation.module.title'], DocumentationUrlBuilder::home());

		$categories = array_reverse(CategoriesService::get_categories_manager('documentation')->get_parents($this->get_category()->get_id(), true));
		foreach ($categories as $id => $category)
		{
			if ($category->get_id() != Category::ROOT_CATEGORY)
				$breadcrumb->add($category->get_name(), DocumentationUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name(), ($category->get_id() == $this->get_category()->get_id() ? $page : 1)));
		}
