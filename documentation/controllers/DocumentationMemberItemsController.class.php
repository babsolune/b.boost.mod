...
class DocumentationMemberItemsController extends DefaultModuleController
{
	private $member;
	...
	private function get_pagination($condition, $parameters, $page)
	{
		...
		$pagination->set_url(DocumentationUrlBuilder::display_member_items($this->get_member()->get_id(), '%d'));
	...
	{
		$page = $request->get_getint('page', 1);
		$page_title = $this->is_current_member_displayed() ? $this->lang['documentation.my.items'] : $this->lang['documentation.member.items'] . ' ' . $this->get_member()->get_display_name();
		$response = new SiteDisplayResponse($this->view);

		$graphical_environment = $response->get_graphical_environment();
		$graphical_environment->set_page_title($page_title, $this->lang['documentation.module.title'], $page);
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['documentation.seo.description.member'], array('author' => $this->get_member()->get_display_name())), $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(DocumentationUrlBuilder::display_member_items($this->get_member()->get_id(), $page));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['documentation.module.title'], DocumentationUrlBuilder::home());
		$breadcrumb->add($page_title, DocumentationUrlBuilder::display_member_items($this->get_member()->get_id(), $page));
