...
class AdminDocumentationConfigController extends DefaultAdminModuleController
{
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		{
			$this->save();
			$this->form->get_field_by_id('display_summary_to_guests')->set_hidden($this->config->get_display_type() == DocumentationConfig::TABLE_VIEW);
			$this->form->get_field_by_id('auto_cut_characters_number')->set_hidden($this->config->get_display_type() == DocumentationConfig::TABLE_VIEW);
			$this->form->get_field_by_id('items_per_row')->set_hidden($this->config->get_display_type() !== DocumentationConfig::GRID_VIEW);
	...
	private function build_form()
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->lang['form.display.type'], $this->config->get_display_type(),
			array(
				new FormFieldSelectChoiceOption($this->lang['form.display.type.grid'], DocumentationConfig::GRID_VIEW, array('data_option_icon' => 'fa fa-th-large')),
				new FormFieldSelectChoiceOption($this->lang['form.display.type.list'], DocumentationConfig::LIST_VIEW, array('data_option_icon' => 'fa fa-list')),
				new FormFieldSelectChoiceOption($this->lang['form.display.type.table'], DocumentationConfig::TABLE_VIEW, array('data_option_icon' => 'fa fa-table'))
			),
			array(
				'select_to_list' => true,
				'events' => array('change' => '
				if (HTMLForms.getField("display_type").getValue() == \'' . DocumentationConfig::GRID_VIEW . '\') {
				...
				} else if (HTMLForms.getField("display_type").getValue() == \'' . DocumentationConfig::LIST_VIEW . '\') {
					HTMLForms.getField("display_summary_to_guests").enable();
		...
		$fieldset->add_field(new FormFieldNumberEditor('items_per_row', $this->lang['form.items.per.row'], $this->config->get_items_per_row(),
			array(
				...
				'hidden' => $this->config->get_display_type() !== DocumentationConfig::GRID_VIEW
			),
			array(new FormFieldConstraintIntegerRange(1, 4))
		));
		...
		$fieldset->add_field(new FormFieldNumberEditor('auto_cut_characters_number', $this->lang['form.characters.number.to.cut'], $this->config->get_auto_cut_characters_number(),
			array(
				...
				'hidden' => $this->config->get_display_type() === DocumentationConfig::TABLE_VIEW
			),
		...
		$auth_settings = new AuthorizationsSettings(array_merge(RootCategory::get_authorizations_settings(), array(
			new ActionAuthorization($this->lang['documentation.manage.archives'], DocumentationAuthorizationsService::MANAGE_ARCHIVES_AUTHORIZATIONS)
		)));
	...
	private function save()
	{
		...
		if($this->form->get_value('display_type') == DocumentationConfig::GRID_VIEW)
		...
		if ($this->config->get_display_type() != DocumentationConfig::TABLE_VIEW)
		{
