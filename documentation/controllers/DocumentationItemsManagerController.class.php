...
class DocumentationItemsManagerController extends DefaultModuleController
{
	...
	private function build_table()
	{
		...
		$table_model = new SQLHTMLTableModel(DocumentationSetup::$documentation_items_table, 'items-manager', $columns, new HTMLTableSortingRule('creation_date', HTMLTableSortingRule::DESC));

		$table_model->set_layout_title($this->lang['documentation.items.management']);
		$table_model->add_permanent_filter('c.active_content = 1');

		$table_model->set_filters_menu_title($this->lang['documentation.filter.items']);
		...
		$result = $table_model->get_sql_results('i
			...
			LEFT JOIN ' . DB_TABLE_NOTE . ' note ON note.id_in_module = i.id AND note.module_name = \'documentation\' AND note.user_id = ' . AppContext::get_current_user()->get_id() . '
		...
		foreach ($result as $row)
		{
			...
			$edit_link = new EditLinkHTMLElement(DocumentationUrlBuilder::edit($item->get_id()));
			$delete_link = new DeleteLinkHTMLElement(DocumentationUrlBuilder::delete($item->get_id(), 0));

			$user_group_color = User::get_group_color($user->get_groups(), $user->get_level(), true);
			$author = $user->get_id() !== User::VISITOR_LEVEL ? new LinkHTMLElement(UserUrlBuilder::profile($user->get_id()), $user->get_display_name(), (!empty($user_group_color) ? array('style' => 'color: ' . $user_group_color) : array()), UserService::get_level_class($user->get_level())) : $user->get_display_name();

			$row = array(
				new HTMLTableRowCell(new LinkHTMLElement(DocumentationUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), $item->get_item_content()->get_title()), 'left'),
				new HTMLTableRowCell(new LinkHTMLElement(DocumentationUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name()), ($category->get_id() == Category::ROOT_CATEGORY ? $this->lang['common.none.alt'] : $category->get_name()))),
        ...
        if ($request->get_string('delete-selected-elements', false))
        {
            for ($i = 1; $i <= $this->elements_number; $i++)
            {
                if ($request->get_value('delete-checkbox-' . $i, 'off') == 'on')
                {
                    if (isset($this->ids[$i]))
                    {
						$item = DocumentationService::get_item($this->ids[$i]);
                        DocumentationService::delete($this->ids[$i], 0);
            ...
            AppContext::get_response()->redirect(DocumentationUrlBuilder::manage(), $this->lang['warning.process.success']);
        }
    }

	private function check_authorizations()
	{
		if (!DocumentationAuthorizationsService::check_authorizations()->moderation())
		{
	...
	private function generate_response($page = 1)
	{
		...
		$graphical_environment->set_page_title($this->lang['documentation.items.management'], $this->lang['documentation.module.title'], $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(DocumentationUrlBuilder::manage());

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['documentation.module.title'], DocumentationUrlBuilder::home());

		$breadcrumb->add($this->lang['documentation.items.management'], DocumentationUrlBuilder::manage());
