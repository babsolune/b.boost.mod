...
class DocumentationReorderItemsController extends DefaultModuleController
{
	private $category;

	protected function get_template_to_use()
	{
		return new FileTemplate('documentation/DocumentationReorderItemsController.tpl');
	}
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		if ($request->get_value('submit', false))
		{
			...
			AppContext::get_response()->redirect(DocumentationUrlBuilder::display_category($this->get_category()->get_id(), $this->get_category()->get_rewrited_name()), $this->lang['warning.success.position.update']);
		}
	...
	private function update_position(HTTPRequestCustom $request)
	{
		...
		{
			DocumentationService::update_position($tree->id, $position);
		}
	}
	...
	private function generate_response()
	{
		...
		if ($this->get_category()->get_id() != Category::ROOT_CATEGORY)
			$graphical_environment->set_page_title($this->get_category()->get_name(), $this->lang['documentation.module.title']);
		else
			$graphical_environment->set_page_title($this->lang['documentation.module.title']);

		$description = $this->get_category()->get_description() . ' ' . $this->lang['items.reorder'];
		if (empty($description))
			$description = StringVars::replace_vars($this->lang['documentation.seo.description.root'], array('site' => GeneralConfig::load()->get_site_name())) . ($this->get_category()->get_id() != Category::ROOT_CATEGORY ? ' ' . $this->lang['category.category'] . ' ' . $this->get_category()->get_name() : '') . ' ' . $this->lang['documentation.questions.reorder'];
		$graphical_environment->get_seo_meta_data()->set_description($description);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(DocumentationUrlBuilder::display_category($this->get_category()->get_id(), $this->get_category()->get_rewrited_name()));
		...
		$breadcrumb->add($this->lang['items.reorder'], DocumentationUrlBuilder::reorder_items($this->get_category()->get_id(), $this->get_category()->get_rewrited_name()));
