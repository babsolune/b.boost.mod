...
class DocumentationDeleteItemController extends DefaultModuleController
{
	public function execute(HTTPRequestCustom $request)
	{
		...
		if($request->get_int('content_id') == 0)
		{
			...
			{
				PersistenceContext::get_querier()->delete(DocumentationSetup::$documentation_contents_table, 'WHERE item_id = :id', array('id' => $this->item->get_id()));
			}
		}
		DocumentationService::delete($this->item->get_id(), $request->get_int('content_id'));
		...
		AppContext::get_response()->redirect((
			$request->get_url_referrer() && !TextHelper::strstr(
				...
				DocumentationUrlBuilder::display($this->item->get_category()->get_id(), $this->item->get_category()->get_rewrited_name(), $this->item->get_id(), $this->item->get_rewrited_title())->rel()) ? $request->get_url_referrer() : DocumentationUrlBuilder::home()),
				$request->get_int('content_id') === 0 ?
					StringVars::replace_vars($this->lang['documentation.message.success.delete'], array('title' => $this->item->get_item_content()->get_title())) :
					StringVars::replace_vars($this->lang['documentation.message.success.delete.content'], array('content' => $request->get_int('content_id'),'title' => $this->item->get_item_content()->get_title())
		));
	}
