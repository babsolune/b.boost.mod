...
class DocumentationAuthorizationsService extends CategoriesAuthorizationsService
{
	...
	protected function is_authorized($bit, $mode = Authorizations::AUTH_CHILD_PRIORITY)
	{
		if ($bit == self::MANAGE_ARCHIVES_AUTHORIZATIONS)
			$auth = DocumentationConfig::load()->get_authorizations();
		else
			$auth = CategoriesService::get_categories_manager('documentation')->get_heritated_authorizations($this->id_category, $bit, $mode);
