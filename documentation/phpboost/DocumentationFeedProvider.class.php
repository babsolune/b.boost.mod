...
class DocumentationFeedProvider implements FeedProvider
{
	public function get_feeds_list()
	{
		return CategoriesService::get_categories_manager('documentation')->get_feeds_categories_module()->get_feed_list();
	}
	...
	public function get_feed_data_struct($idcat = 0, $name = '')
	{
		...
		if (CategoriesService::get_categories_manager($module_id)->get_categories_cache()->category_exists($idcat))
		{
			...
			$feed_module_name = LangLoader::get_message('documentation.module.title', 'common', 'documentation');
			$data = new FeedData();
			$data->set_title($feed_module_name . ' - ' . $site_name);
			$data->set_date(new Date());
			$data->set_link(SyndicationUrlBuilder::rss('documentation', $idcat));
			$data->set_host(HOST);
			...
			$now = new Date();
			$results = $querier->select('SELECT documentation.id, documentation.id_category, documentation.title, documentation.rewrited_title, documentation.content, documentation.update_date, documentation.thumbnail, cat.rewrited_name AS rewrited_name_cat
				FROM ' . DocumentationSetup::$documentation_items_table . ' documentation
				LEFT JOIN '. DocumentationSetup::$documentation_cats_table .' cat ON cat.id = documentation.id_category
				WHERE documentation.id_category IN :ids_categories
				AND (published = 1 OR (published = 2 AND publishing_start_date < :timestamp_now AND (publishing_end_date > :timestamp_now OR publishing_end_date = 0)))
				ORDER BY documentation.update_date DESC', array(
			...
			foreach ($results as $row)
			{
				...
				$link = DocumentationUrlBuilder::display($row['id_category'], $row['rewrited_name_cat'], $row['id'], $row['rewrited_title']);
