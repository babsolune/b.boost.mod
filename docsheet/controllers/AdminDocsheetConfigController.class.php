...
class AdminDocsheetConfigController extends DefaultAdminModuleController
{
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		{
			$this->save();
			$this->form->get_field_by_id('display_summary_to_guests')->set_hidden($this->config->get_display_type() == DocsheetConfig::TABLE_VIEW);
			$this->form->get_field_by_id('auto_cut_characters_number')->set_hidden($this->config->get_display_type() == DocsheetConfig::TABLE_VIEW);
			$this->form->get_field_by_id('items_per_row')->set_hidden($this->config->get_display_type() !== DocsheetConfig::GRID_VIEW);
	...
	private function build_form()
	{
		...
		$fieldset->add_field(new FormFieldTextEditor('module_name', $this->lang['docsheet.name'], $this->config->get_module_name()));

		$fieldset->add_field(new FormFieldCheckbox('display_sticky_summary', $this->lang['docsheet.sticky.contents.table'], $this->config->get_sticky_summary(),
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->lang['form.display.type'], $this->config->get_display_type(),
			array(
				new FormFieldSelectChoiceOption($this->lang['form.display.type.grid'], DocsheetConfig::GRID_VIEW, array('data_option_icon' => 'fa fa-th-large')),
				new FormFieldSelectChoiceOption($this->lang['form.display.type.list'], DocsheetConfig::LIST_VIEW, array('data_option_icon' => 'fa fa-list')),
				new FormFieldSelectChoiceOption($this->lang['form.display.type.table'], DocsheetConfig::TABLE_VIEW, array('data_option_icon' => 'fa fa-table'))
			),
			array(
				'select_to_list' => true,
				'events' => array('change' => '
				if (HTMLForms.getField("display_type").getValue() == \'' . DocsheetConfig::GRID_VIEW . '\') {
				...
				} else if (HTMLForms.getField("display_type").getValue() == \'' . DocsheetConfig::LIST_VIEW . '\') {
					HTMLForms.getField("display_summary_to_guests").enable();
		...
		$fieldset->add_field(new FormFieldNumberEditor('items_per_row', $this->lang['form.items.per.row'], $this->config->get_items_per_row(),
			array(
				...
				'hidden' => $this->config->get_display_type() !== DocsheetConfig::GRID_VIEW
			),
			array(new FormFieldConstraintIntegerRange(1, 4))
		));
		...
		$fieldset->add_field(new FormFieldNumberEditor('auto_cut_characters_number', $this->lang['form.characters.number.to.cut'], $this->config->get_auto_cut_characters_number(),
			array(
				...
				'hidden' => $this->config->get_display_type() === DocsheetConfig::TABLE_VIEW
			),
		...
		$fieldset->add_field(new FormFieldNumberEditor('suggested_items_nb', $this->lang['docsheet.suggestions.number'], $this->config->get_suggested_items_nb(),
			array(
				'min' => 1, 'max' => 10,
		...
		$auth_settings = new AuthorizationsSettings(array_merge(RootCategory::get_authorizations_settings(), array(
			new VisitorDisabledActionAuthorization($this->lang['docsheet.manage.archives'], DocsheetAuthorizationsService::MANAGE_ARCHIVES_AUTHORIZATIONS)
		)));
	...
	private function save()
	{
		...
		if($this->form->get_value('display_type') == DocsheetConfig::GRID_VIEW)
		...
		if ($this->config->get_display_type() != DocsheetConfig::TABLE_VIEW)
		{
