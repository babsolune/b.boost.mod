...
class DocsheetPendingItemsController extends DefaultModuleController
{
	...
	public function build_view(HTTPRequestCustom $request)
	{
		...
		' . $condition .'
		' . (!DocsheetAuthorizationsService::check_authorizations()->write() ? ' AND c.author_user_id = :user_id' : '') . '
	...
	private function get_pagination($condition, $parameters, $page)
	{
		...
		$pagination->set_url(DocsheetUrlBuilder::display_pending('%d'));
	...
	private function generate_response(HTTPRequestCustom $request)
	{
		...
		$graphical_environment->set_page_title($this->lang['docsheet.pending.items'], $this->config->get_module_name(), $page);
		$graphical_environment->get_seo_meta_data()->set_description($this->lang['docsheet.seo.description.pending'], $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(DocsheetUrlBuilder::display_pending($page));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->config->get_module_name(), DocsheetUrlBuilder::home());
		$breadcrumb->add($this->lang['docsheet.pending.items'], DocsheetUrlBuilder::display_pending($page));
