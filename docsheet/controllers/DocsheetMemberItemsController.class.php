...
class DocsheetMemberItemsController extends DefaultModuleController
{
	private $member;
	...
	private function get_pagination($condition, $parameters, $page)
	{
		...
		$pagination->set_url(DocsheetUrlBuilder::display_member_items($this->get_member()->get_id(), '%d'));
	...
	{
		$page = $request->get_getint('page', 1);
		$page_title = $this->is_current_member_displayed() ? $this->lang['docsheet.my.items'] : $this->lang['docsheet.member.items'] . ' ' . $this->get_member()->get_display_name();
		$response = new SiteDisplayResponse($this->view);

		$graphical_environment = $response->get_graphical_environment();
		$graphical_environment->set_page_title($page_title, $this->config->get_module_name(), $page);
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['docsheet.seo.description.member'], array('author' => $this->get_member()->get_display_name())), $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(DocsheetUrlBuilder::display_member_items($this->get_member()->get_id(), $page));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->config->get_module_name(), DocsheetUrlBuilder::home());
		$breadcrumb->add($page_title, DocsheetUrlBuilder::display_member_items($this->get_member()->get_id(), $page));
