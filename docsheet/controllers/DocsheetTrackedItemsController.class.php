...
class DocsheetTrackedItemsController extends DefaultModuleController
{
	...
	public function build_view(HTTPRequestCustom $request)
	{
		...
		LEFT JOIN ' . DocsheetSetup::$docsheet_favorites_table . ' t ON t.track_item_id = i.id
		...
		$this->view->put_all(array(
			...
			'C_CONTROLS'             => DocsheetAuthorizationsService::check_authorizations()->read(),
			...
			'TABLE_COLSPAN'      => 4 + (int)$comments_config->module_comments_is_enabled('docsheet') + (int)$content_management_config->module_notation_is_enabled('docsheet'),
		));
	...
	private function get_pagination($condition, $parameters, $page)
	{
		...
		$pagination->set_url(DocsheetUrlBuilder::tracked_member_items(AppContext::get_current_user()->get_id(), '%d'));
	...
	{
		$page = $request->get_getint('page', 1);
		$page_title = $this->lang['docsheet.my.tracked'];
		$response = new SiteDisplayResponse($this->view);

		$graphical_environment = $response->get_graphical_environment();
		$graphical_environment->set_page_title($page_title, $this->config->get_module_name(), $page);
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['docsheet.seo.description.tracked'], array('author' => AppContext::get_current_user()->get_display_name())), $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(DocsheetUrlBuilder::tracked_member_items(AppContext::get_current_user()->get_id(), $page));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->config->get_module_name(), DocsheetUrlBuilder::home());
		$breadcrumb->add($page_title, DocsheetUrlBuilder::tracked_member_items(AppContext::get_current_user()->get_id(), $page));
