...
class DocsheetReorderItemsController extends DefaultModuleController
{
	private $category;

	protected function get_template_to_use()
	{
		return new FileTemplate('docsheet/DocsheetReorderItemsController.tpl');
	}
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		if ($request->get_value('submit', false))
		{
			...
			AppContext::get_response()->redirect(DocsheetUrlBuilder::display_category($this->get_category()->get_id(), $this->get_category()->get_rewrited_name()), $this->lang['warning.success.position.update']);
		}
	...
	private function build_view(HTTPRequestCustom $request)
	{
		...
		while ($row = $result->fetch())
		{
			...
			$this->view->assign_block_vars('items', array(
                ...
                'U_EDIT' => DocsheetUrlBuilder::edit($item->get_id())->rel(),
                'U_DELETE' => DocsheetUrlBuilder::delete($item->get_id(), 0)->rel(),
            ));
		}
		$result->dispose();
	}
	...
	private function update_position(HTTPRequestCustom $request)
	{
		...
		{
			DocsheetService::update_position($tree->id, $position);
		}
	}
	...
	private function generate_response()
	{
		...
		if (empty($description))
			$description = StringVars::replace_vars($this->lang['docsheet.seo.description.root'], array('site' => GeneralConfig::load()->get_site_name())) . ($this->get_category()->get_id() != Category::ROOT_CATEGORY ? ' ' . $this->lang['category.category'] . ' ' . $this->get_category()->get_name() : '') . ' ' . $this->lang['docsheet.questions.reorder'];
		...
		$breadcrumb->add($this->lang['items.reorder'], DocsheetUrlBuilder::reorder_items($this->get_category()->get_id(), $this->get_category()->get_rewrited_name()));
