...
class DocsheetItemFormController extends DefaultModuleController
{
	public function execute(HTTPRequestCustom $request)
	{
		...
		{
			$this->form->get_field_by_id('custom_level')->set_hidden($item_content->get_content_level() != DocsheetItemContent::CUSTOM_LEVEL);
			$this->save();
			$this->redirect();
		}
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		$form->set_layout_title($this->get_item()->get_id() === null ? $this->lang['docsheet.add.item'] : ($this->lang['docsheet.edit.item']));

		$fieldset = new FormFieldsetHTML('docsheet', $this->lang['form.parameters']);
		$form->add_fieldset($fieldset);
		if ($this->get_item()->get_id() !== null)
			$fieldset->set_description($this->lang['docsheet.message.draft']);
		...
		$fieldset->add_field(new FormFieldCheckbox('summary_enabled', $this->lang['form.enable.summary'], $item_content->is_summary_enabled(),
			array(
				'description' => StringVars::replace_vars($this->lang['form.summary.clue'], array('number' => DocsheetConfig::load()->get_auto_cut_characters_number())),
				'events' => array('click' => '
		...
		$options_fieldset->add_field(new FormFieldSimpleSelectChoice('content_level', $this->lang['docsheet.level'], $item_content->get_content_level(),
			array(
				new FormFieldSelectChoiceOption('', DocsheetItemContent::NO_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['docsheet.level.wip'], DocsheetItemContent::WIP_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['docsheet.level.sketch'], DocsheetItemContent::SKETCH_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['docsheet.level.redo'], DocsheetItemContent::REDO_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['docsheet.level.claim'], DocsheetItemContent::CLAIM_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['docsheet.level.trust'], DocsheetItemContent::TRUST_LEVEL),
				new FormFieldSelectChoiceOption($this->lang['docsheet.level.custom'], DocsheetItemContent::CUSTOM_LEVEL),
			),
			array(
				'events' => array('change' => '
					if (HTMLForms.getField("content_level").getValue() == \'' . DocsheetItemContent::CUSTOM_LEVEL . '\') {
		...
		$options_fieldset->add_field(new FormFieldRichTextEditor('custom_level', $this->lang['docsheet.level.custom.content'], $item_content->get_custom_level(),
			array('hidden' => $item_content->get_content_level() != DocsheetItemContent::CUSTOM_LEVEL)
		));

		$options_fieldset->add_field(new FormFieldThumbnail('thumbnail', $this->lang['form.thumbnail'], $item_content->get_thumbnail()->relative(), DocsheetItem::THUMBNAIL_URL));
		...
		if(!$this->is_new_item)
		{
			$options_fieldset->add_field(new FormFieldMultiLineTextEditor('change_reason', $this->lang['docsheet.change.reason'], '',
				array('rows' => 5)
			));
		}

		if (DocsheetAuthorizationsService::check_authorizations($this->get_item()->get_id_category())->write())
		{
			...
			if (!$this->get_item()->is_published())
			{
				$publication_fieldset->add_field(new FormFieldCheckbox('update_creation_date', $this->lang['form.update.creation.date'], false,
					array('hidden' => $this->get_item()->get_publishing_state() != DocsheetItem::NOT_PUBLISHED)
				));
			}

			$publication_fieldset->add_field(new FormFieldSimpleSelectChoice('published', $this->lang['form.publication'], $this->get_item()->get_id() === null ? $this->get_item()->get_publishing_state() : DocsheetItem::NOT_PUBLISHED,
				array(
					new FormFieldSelectChoiceOption($this->lang['form.publication.draft'], DocsheetItem::NOT_PUBLISHED),
					new FormFieldSelectChoiceOption($this->lang['form.publication.now'], DocsheetItem::PUBLISHED),
					new FormFieldSelectChoiceOption($this->lang['form.publication.deffered'], DocsheetItem::DEFERRED_PUBLICATION),
				),
				array(
			...
			$publication_fieldset->add_field($publishing_start_date = new FormFieldDateTime('publishing_start_date', $this->lang['form.start.date'], ($this->get_item()->get_publishing_start_date() === null ? new Date() : $this->get_item()->get_publishing_start_date()),
				array('hidden' => ($request->is_post_method() ? ($request->get_postint(__CLASS__ . '_publication_state', 0) != DocsheetItem::DEFERRED_PUBLICATION) : ($this->get_item()->get_publishing_state() != DocsheetItem::DEFERRED_PUBLICATION)))
			));
			...
			$publication_fieldset->add_field(new FormFieldCheckbox('end_date_enabled', $this->lang['form.enable.end.date'], $this->get_item()->is_end_date_enabled(),
				array(
					'hidden' => ($request->is_post_method() ? ($request->get_postint(__CLASS__ . '_publication_state', 0) != DocsheetItem::DEFERRED_PUBLICATION) : ($this->get_item()->get_publishing_state() != DocsheetItem::DEFERRED_PUBLICATION)),
					'events' => array('click' => '
						if (HTMLForms.getField("end_date_enabled").getValue()) {
							HTMLForms.getField("publishing_end_date").enable();
						} else {
	...
	{
		return (!DocsheetAuthorizationsService::check_authorizations()->write() && DocsheetAuthorizationsService::check_authorizations()->contribution());
	}
	...
	private function get_item()
	{
		if ($this->item === null)
		{
			...
			else
			{
				...
				$item_content = new DocsheetItemContent();
	...
	private function save()
	{
		...
		if ($item->get_i_order() === null) {
			$items_number_in_category = DocsheetService::count('WHERE id_category = :id_category', array('id_category' => $this->get_item()->get_id_category()));
		...
		if ($this->form->get_value('content_level')->get_raw_value() == DocsheetItemContent::CUSTOM_LEVEL)
		...
		if (!DocsheetAuthorizationsService::check_authorizations($this->get_item()->get_id_category())->write())
		{
			...
			if (DocsheetAuthorizationsService::check_authorizations($item->get_id_category())->contribution() && !DocsheetAuthorizationsService::check_authorizations($item->get_id_category())->write())
				$item->set_publishing_state(DocsheetItem::NOT_PUBLISHED);
		}
		else
		{
			...
			if ($item->get_publishing_state() == DocsheetItem::DEFERRED_PUBLICATION)
			{
		...
		if ($this->is_new_item)
        {
			$items_number_in_category = DocsheetService::count('WHERE id_category = :id_category', array('id_category' => $item->get_id_category()));
			$item->set_i_order($items_number_in_category + 1);

			$last_content_id = DocsheetService::get_last_content_id();
			foreach ($last_content_id as $content_id) {
				$item_content->set_content_id($content_id + 1);
			}
			$item_content->set_active_content('1');

			$id = DocsheetService::add($item);
			$item->set_id($id);

			$item_content->set_item_id($id);
			$content_id = DocsheetService::add_content($item_content);
        ...
        else
        {
            ...
            if ($item->is_published()) 
            {
                ...
                PersistenceContext::get_querier()->update(PREFIX . "docsheet_contents", array('active_content' => 0), 'WHERE item_id = :item_id', array('item_id' => $this->get_item()->get_id()));
            ...
            {
                $item_content->set_active_content('1');
                $content_id = DocsheetService::update_content($item_content);
            }

            $item_content->set_content_id($content_id);
            $item->set_item_content($item_content);
            DocsheetService::update($item);
		...
		DocsheetService::clear_cache();
	}

	private function contribution_actions(DocsheetItem $item)
	{
		$item_content = $this->get_item()->get_item_content();
		if ($this->is_contributor_member())
		{
			...
			$contribution->set_fixing_url(DocsheetUrlBuilder::edit($item->get_id())->relative());
			$contribution->set_poster_id(AppContext::get_current_user()->get_id());
			$contribution->set_module('docsheet');
		...
		{
			$corresponding_contributions = ContributionService::find_by_criteria('docsheet', $item->get_id());
	...
	private function redirect()
	{
		...
		{
			if ($this->is_new_item)
				AppContext::get_response()->redirect(DocsheetUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), StringVars::replace_vars($this->lang['docsheet.message.success.add'], array('title' => $item_content->get_title())));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : DocsheetUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title())), StringVars::replace_vars($this->lang['docsheet.message.success.edit'], array('title' => $item_content->get_title())));
		}
		else
		{
			if ($this->is_new_item)
				AppContext::get_response()->redirect(DocsheetUrlBuilder::display_pending(), StringVars::replace_vars($this->lang['docsheet.message.success.add'], array('title' => $item_content->get_title())));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : DocsheetUrlBuilder::display_pending()), StringVars::replace_vars($this->lang['docsheet.message.success.edit'], array('title' => $item_content->get_title())));
		}
	}
	...
	private function generate_response(View $view)
	{
		...
		$location_id = $item->get_id() ? 'docsheet-edit-'. $item->get_id() : '';
		...
		if ($item->get_id() === null)
		{
			$breadcrumb->add($this->lang['docsheet.add.item'], DocsheetUrlBuilder::add($item->get_id_category()));
			$graphical_environment->set_page_title($this->lang['docsheet.add.item'], $this->config->get_module_name());
			$graphical_environment->get_seo_meta_data()->set_description($this->lang['docsheet.add.item']);
			$graphical_environment->get_seo_meta_data()->set_canonical_url(DocsheetUrlBuilder::add($item->get_id_category()));
		}
		else
		{
			...
			$graphical_environment->set_page_title($this->lang['docsheet.edit.item'], $this->config->get_module_name());
			$graphical_environment->get_seo_meta_data()->set_description($this->lang['docsheet.edit.item']);
			$graphical_environment->get_seo_meta_data()->set_canonical_url(DocsheetUrlBuilder::edit($item->get_id()));
			...
			$breadcrumb->add($this->lang['docsheet.edit.item'], DocsheetUrlBuilder::edit($item->get_id()));
		}
