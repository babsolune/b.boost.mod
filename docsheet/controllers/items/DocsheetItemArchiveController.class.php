...
class DocsheetItemArchiveController extends DefaultModuleController
{
	...
	private function get_item()
	{
		if ($this->item === null)
		{
			...
			{
				try {
					$this->item = DocsheetService::get_item_archive($id, $content_id);
	...
	private function build_view()
	{
		...
		$this->view->put_all(array_merge($item->get_template_vars(), array(
			...
			'ARCHIVED_CONTENT' => MessageHelper::display($this->lang['docsheet.archived.content'], MessageHelper::WARNING),

			'U_DELETE_CONTENT' => DocsheetUrlBuilder::delete($item->get_id(), $item->get_item_content()->get_content_id())->rel(),
			'U_RESTORE' 	   => DocsheetUrlBuilder::restore($item->get_id(), $item->get_item_content()->get_content_id())->rel()
		)));
	...
	private function generate_response()
	{
		...
		$breadcrumb->add($item->get_item_content()->get_title(), DocsheetUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()));
		$breadcrumb->add($this->lang['docsheet.archive'] . ' ' . $item->get_item_content()->get_content_id(), DocsheetUrlBuilder::archive($item->get_id(), $item->get_item_content()->get_content_id()));
