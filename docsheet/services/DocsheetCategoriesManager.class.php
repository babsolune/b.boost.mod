...
class DocsheetCategoriesManager extends CategoriesManager
{
	/**
	...
	public function delete($id)
	{
		...
		{
            for($i = 0; $i < count(DocsheetService::get_item_content($row['id'])); $i++ )
			{
				PersistenceContext::get_querier()->delete(DocsheetSetup::$docsheet_contents_table, 'WHERE item_id = :id', array('id' => $row['id']));
			}
			DocsheetService::delete($row['id'], 0);
		}
		$result->dispose();
