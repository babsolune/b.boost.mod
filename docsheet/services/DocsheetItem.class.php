...
class DocsheetItem
{
	...
	public function set_item_content(DocsheetItemContent $item_content)
	{
	...
	{
		return CategoriesService::get_categories_manager('docsheet')->get_categories_cache()->get_category($this->id_category);
	}
	...
	{
		$now = new Date();
		return DocsheetAuthorizationsService::check_authorizations($this->id_category)->read() && ($this->get_publishing_state() == self::PUBLISHED || ($this->get_publishing_state() == self::DEFERRED_PUBLICATION && $this->get_publishing_start_date()->is_anterior_to($now) && ($this->end_date_enabled ? $this->get_publishing_end_date()->is_posterior_to($now) : true)));
	}
	...
	{
		return
            DocsheetAuthorizationsService::check_authorizations($this->id_category)->write()
            || DocsheetAuthorizationsService::check_authorizations($this->id_category)->contribution()
        ;
	}
	...
	{
		return
            DocsheetAuthorizationsService::check_authorizations($this->id_category)->moderation()
			|| DocsheetAuthorizationsService::check_authorizations($this->id_category)->write()
            || (
                DocsheetAuthorizationsService::check_authorizations($this->id_category)->contribution()
	...
	public function is_authorized_to_restore()
	{
		return
            DocsheetAuthorizationsService::check_authorizations($this->id_category)->moderation()
            || DocsheetAuthorizationsService::check_authorizations($this->id_category)->manage_archives()
        ;
	}
	...
	public function set_properties(array $properties)
	{
		...
		$notation->set_module_name('docsheet');
	...
	public function get_item_url()
	{
		...
		return DocsheetUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $this->id, $this->rewrited_title)->rel();
	}
	...
	public function get_template_vars()
	{
		...
		$rich_content              = HooksService::execute_hook_display_action('docsheet', $content, $this->get_properties());
		$real_summary              = $this->item_content->get_real_summary();
		$user                      = DocsheetService::get_initial_content($this->id)->get_author_user();
		...
		$comments_number           = CommentsService::get_comments_number('docsheet', $this->id);
		$sources                   = $this->item_content->get_sources();
		$nbr_sources               = count($sources);
		$config                    = DocsheetConfig::load();
        $tracked_list              = DocsheetService::get_tracked_items($this->id);
		...
		return array_merge(
			...
			array(
				...
				'C_NEW_CONTENT'             => ContentManagementConfig::load()->module_new_content_is_enabled_and_check_date('docsheet', $this->get_publishing_start_date() != null ? $this->get_publishing_start_date()->get_timestamp() : $this->creation_date->get_timestamp()) && $this->is_published(),
				'C_INIT'				    => DocsheetService::get_initial_content($this->id)->get_content_id() == $this->item_content->get_content_id(),
				...
				'L_COMMENTS'      => CommentsService::get_lang_comments('docsheet', $this->id),
				...
				'U_EDIT_CATEGORY'      => $category->get_id() == Category::ROOT_CATEGORY ? DocsheetUrlBuilder::configuration()->rel() : CategoriesUrlBuilder::edit($category->get_id(), 'docsheet')->rel(),

				// Links
				'U_SYNDICATION'         => SyndicationUrlBuilder::rss('docsheet', $this->id_category)->rel(),
				...
				'U_TRACK'               => DocsheetUrlBuilder::track_item($this->id, AppContext::get_current_user()->get_id())->rel(),
				'U_UNTRACK'             => DocsheetUrlBuilder::untrack_item($this->id, AppContext::get_current_user()->get_id())->rel(),
				'U_HISTORY'             => DocsheetUrlBuilder::history($this->id)->rel(),
				'U_EDIT'                => DocsheetUrlBuilder::edit($this->id)->rel(),
				'U_DELETE'              => DocsheetUrlBuilder::delete($this->id, 0)->rel(),
				'U_THUMBNAIL'           => $this->item_content->get_thumbnail()->rel(),
				'U_COMMENTS'            => DocsheetUrlBuilder::display_comments($category->get_id(), $category->get_rewrited_name(), $this->id, $this->rewrited_title)->rel()
			)
		);
	}
