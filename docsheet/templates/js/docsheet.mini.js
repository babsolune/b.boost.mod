...
jQuery(document).ready(function () {
    jQuery('#docsheet-mini-nav').append(DocsheetMiniCreateChild(0)).find('ul:first').remove();

	function DocsheetMiniCreateChild(id){
		var $li = jQuery('li[data-docsheet-parent-id="' + id + '"]').sort(function(a, b){
			return jQuery(a).attr('data-docsheet-order-id') - jQuery(b).attr('data-docsheet-order-id');
		});
		if($li.length > 0){
			for(var i = 0; i < $li.length; i++){
				var $this = $li.eq(i);
                $this.append(DocsheetMiniCreateChild($this.attr('data-docsheet-id')));
			}
            return jQuery('<ul class="level-' + id + '">').append($li);
		}
    }

    jQuery('#docsheet-mini-nav .items-list li').each(function() {
