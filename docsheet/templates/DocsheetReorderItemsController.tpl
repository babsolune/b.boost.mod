# IF C_ITEMS #
	<script>
		...
		var DocsheetItem = function(id, docsheet){
			this.id = id;
			this.DocsheetItems = docsheet;

			if (DocsheetItems.items_number > 1)
				DocsheetItems.change_reposition_pictures();
		};

		var DocsheetItems = new DocsheetItems('items-list');
		jQuery(document).ready(function() {
			DocsheetItems.init_sortable();
		});
	</script>
# ENDIF #
# INCLUDE MESSAGE_HELPER #
<section id="module-docsheet">
	<header class="section-header">
		<div class="controls align-right">
			<a class="offload" href="${relative_url(SyndicationUrlBuilder::rss('docsheet', CATEGORY_ID))}" aria-label="{@common.syndication}"><i class="fa fa-rss warning" aria-hidden="true"></i></a>
	...
	# IF C_ITEMS #
		<div class="sub-section">
			<div class="content-container">
				<div class="content">
					<form action="{REWRITED_SCRIPT}" method="post" id="position-update-form" onsubmit="DocsheetItems.serialize_sortable();" class="docsheet-reorder-form">
							...
							<ul id="items-list" class="sortable-block">
								# START items #
									<li class="sortable-element# IF items.C_NEW_CONTENT # new-content# ENDIF #" id="list-{items.ID}" data-id="{items.ID}">
										...
										<script>
											jQuery(document).ready(function() {
												var docsheet_question = new DocsheetItem({items.ID}, DocsheetItems);
