...
class DocsheetFeedProvider implements FeedProvider
{
	public function get_feeds_list()
	{
		return CategoriesService::get_categories_manager('docsheet')->get_feeds_categories_module()->get_feed_list();
	}
	...
	public function get_feed_data_struct($idcat = 0, $name = '')
	{
		...
		if (CategoriesService::get_categories_manager($module_id)->get_categories_cache()->category_exists($idcat))
		{
			...
			$feed_module_name = LangLoader::get_message('docsheet.module.title', 'common', 'docsheet');
			$data = new FeedData();
			$data->set_title($feed_module_name . ' - ' . $site_name);
			$data->set_date(new Date());
			$data->set_link(SyndicationUrlBuilder::rss('docsheet', $idcat));
			$data->set_host(HOST);
			...
			$results = $querier->select('SELECT i.id, i.id_category, c.title, i.rewrited_title, c.content, c.update_date, c.thumbnail, cat.rewrited_name AS rewrited_name_cat
				...
				LEFT JOIN '. DocsheetSetup::$docsheet_cats_table .' cat ON cat.id = i.id_category
			...
			foreach ($results as $row)
			{
				...
				$link = DocsheetUrlBuilder::display($row['id_category'], $row['rewrited_name_cat'], $row['id'], $row['rewrited_title']);
